        ...
        <el-row>
          <el-col :span="10">
            <el-form-item label="用户信息">
              <el-input placeholder="用户ID/姓名/手机号/邮箱" v-model="searchConditionData.userInfo"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item label="订单信息">
              <el-input placeholder="商品订单号/平台支付流水号/商品名称" v-model="searchConditionData.orderInfo"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
      ...
      <el-table
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button
              plain
              v-if="checkServiceState(scope.row)"
              @keydown.native.prevent
              @click="tableServiceBtnOnClick(scope.row)"
            >
              开通服务
            </el-button>
          </template>
 ...
 * 功能描述：商品订单列表2.0
 * 参数说明：
 * 注意事项：与1.0的区别在于支付方式为余额支付，且支付单位为唯币；
 *         两者共有一个接口，通过字段 currencyType 区分，支付币类型：0->现金；1->余额（唯币）
 *
 * Create by YaoQiao on 2019-06-25
 */
...
export default {
  ...
  data() {
    return {
      ...
      searchConditionData: {
        ...
        payState: '', // 支付状态
        refundState: '', // 退款情况
        currencyType: 1 // 支付币类型：0->现金；1->余额（唯币）
      },
  ...
  methods: {
    ...
    checkServiceState(row) {
      // 校验是否可以显示"开通服务"按钮
      // 当且仅当已支付成功但未发货成功，且支付终端为 Android 才会显示
      if (row.payState === 3 && row.orderState === 2 && row.sourceType === 2) {
        return true;
      }
      return false;
    },
    tableServiceBtnOnClick(row) {
      // 开通服务按钮点击处理函数
      ...
      this.$allin_confirm({
        ...
        content: '是否确认开课？',
          ...
          axios({
            method: apiUrlConfig.accessCourseRight.type,
            url: apiUrlConfig.accessCourseRight.url,
            data: data
          }).then((res) => {
            comm.closeLoading();
            if (res && res.data && res.data.responseObject && res.data.responseObject.responseCode === '0') {
              this.$message('开课成功');
              this.getDataList();
            }
            else {
