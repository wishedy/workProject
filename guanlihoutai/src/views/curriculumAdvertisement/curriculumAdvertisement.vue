<template>
  <section class="currAdConent">
    <!--title位置-->
    <div class="TopTitle">首页 > 课程广告管理</div>
    <!--筛选box区域-->
  ...
  methods: {
    ...
    popupShowFn(type, row) { // type:区分新建（1）还是编辑（2），新建有确认按钮，编辑没有。
      ...
      if (type === 1) { // 如果是新建则将所有数据进行初始化
        ...
        // 当点击编辑按钮后点击新建按钮时，之前存在的数据不会清除，加上serTimeout可解决
        setTimeout(function() {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let params = {
            ...
            recommendType: 2, // 首页推荐类型(1:推荐广告，2:课程广告，3:直播广告。新增广告的时候必须传入）
    ...
    getListFn() {
      ...
      let param = {
        ...
        recommendType: 2, // 首页推荐类型(1:推荐广告，2:课程广告，3:直播广告)
