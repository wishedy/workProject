<template>
  <section class="currAdConent">
    <!--title位置-->
    <div class="TopTitle">首页 > 唯医学院广告管理</div>
    <!--筛选box区域-->
    ...
    <section>
      ...
      >
        <img
          :src="bigImgPath"
          class="allinSchoolImg"
        >
        <span v-if="bigImgTitle" class="allinSchoolTxt">{{bigImgTitle}}</span>
      </div>
    </section>
    <!--编辑弹层-->
    <el-dialog title="新增/编辑广告" :visible.sync="dialogFormVisible" width="770px">
      <el-form :model="descForm" :rules="descRules" ref="descForm">
        ...
        <el-form-item label="1035：390配图" prop="imgUrl" :label-width="formLabelWidth" class="imgUrl">
          <img class="adImg1035" :src="descForm.imgUrl" alt="">
          <span class="tipTxt">适用于唯医学院banner</span>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getListFn() {
      // 传入的参数全部为点击筛选之后的参数
      let param = {
        recommendType: this.recommendType, // 首页推荐类型(1:推荐广告，2:课程广告，3:直播广告 5:学院广告)
      ...
      let callbacks = (res) => {
        ...
        this.tableData.map((item) => {
          item.bannerImage = this.getImageList(item.imageList, 1);
        });
    ...
    getDescFn() {
      ...
      let callbacks = (res) => {
        if (res && res.data && res.data.responseObject && res.data.responseObject.items.length > 0) {
          ...
          this.descForm.resourceId = row.resourceId; // 资源id
          this.descForm.linkUrl = row.linkUrl; // 链接地址
          this.descForm.imgUrl = this.getImageList(row.imageList, 1); // 链接地址
          // 提交的内容
