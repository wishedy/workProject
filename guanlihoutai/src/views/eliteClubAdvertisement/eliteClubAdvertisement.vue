<template>
  <section class="currAdConent">
    <!--title位置-->
    <div class="TopTitle">首页 > 菁英会广告管理</div>
    <!--筛选box区域-->
    ...
    <section>
        ...
        >
          <template slot-scope="scope">
            <img
              ...
              class="bigImgShow"
              alt=""
              v-if="scope.row.specialImageUrl"
      ...
      >
        <img
          ...
          class="eliteImg"
        >
        <span v-if="bigImgTitle" class="eliteTxt">{{bigImgTitle}}</span>
      </div>
    </section>
    <!--编辑弹层-->
    <el-dialog title="新增/编辑广告" :visible.sync="dialogFormVisible" width="770px">
      <el-form :model="descForm" :rules="descRules" ref="descForm">
        ...
        <el-form-item label="335:100配图" prop="imgUrl" :label-width="formLabelWidth" class="imgUrl">
          <img class="adImg335" :src="descForm.imgUrl" alt="">
        </el-form-item>
...
<script>
/**
   * CRM 管理后台，首页广告管理——精英会广告管理
   * 注意事项：课程管理、推荐管理、直播管理、经营户管理共用一个sass文件，修改时需要注意会不会影响其他样式，功能相似
   * author:zhanghongda
   * create-time:2019.03.11
   * 产品版本：新后台系统v1.6.0（大版本3.5）
   * 功能信息：
   *  1，列表展示
   *  2，列表筛选、排序
   *  3，添加新的广告
...
export default {
  ...
  methods: {
    ...
    smallImgMouseenter(row) {
      ...
      if (row.specialImageUrl) {
        this.bigImgDialogVisible = true;
        this.bigImgPath = row.specialImageUrl;
    ...
    popupShowFn(type, row) { // type:区分新建（1）还是编辑（2），新建有确认按钮，编辑没有。
      ...
      if (type === 1) { // 如果是新建则将所有数据进行初始化
        ...
        setTimeout(function() {
          ...
          this.descForm.imgUrl = row.specialImageUrl;// 图片地址
    ...
    getDescFn() {
      ...
      let callbacks = (res) => {
        if (res && res.data && res.data.responseObject && res.data.responseObject.items.length > 0) {
          ...
          this.descForm.resourceId = row.resourceId;// 资源id
          this.descForm.linkUrl = row.linkUrl;// 链接地址
          this.descForm.imgUrl = row.specialImageUrl;// 链接地址
          // 提交的内容
