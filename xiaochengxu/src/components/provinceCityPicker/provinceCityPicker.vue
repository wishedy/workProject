<template>
  <div>
    ...
    >
      <view class="picker">
        <div class="left" :class="{'error-tip-color':cityErrorTip=='province'}">所在城市</div><div class="right">{{selectedResult}}</div>
      </view>
    </picker>
  </div>
</template>
  ...
  export default {
    ...
    computed:{
      selectedResult(){
        ...
        if(this.provinceId==""){ // 尚未选择
          return "请选择"
        }else{
          return  (this.provinceName.length>5?this.provinceName.substr(0,5)+'...':this.provinceName)
          + ","
          + (this.cityName.length>5?this.cityName.substr(0,5)+'...':this.cityName);
        }
      }
    },
    mounted() {
    ...
    onLoad(){
      ...
      if(!this.defaultProvince && !this.defaultCity){   // 没有初始值的话
        console.log("reinit")
    ...
    methods: {
      ...
      initProvinceAndCity(){
        ...
        for (let i = 0; i < provinces.length; i++) {  // 遍历省份
        ...
        if(this.cityName=="县"){ // 产品让把直辖市下定位到的县，直接改为市。不用选择到县
          _this.cityId = provinces[_this.provinceIndex].cities[0][0];
          _this.cityName = provinces[_this.provinceIndex].cities[0][1];
        }else{
          // 遍历城市
