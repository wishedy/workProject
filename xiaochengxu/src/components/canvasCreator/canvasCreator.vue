<template>
  ...
  <canvas canvasId="myCanvas" class="canvas" :class="{'debug' : params.debug,'pro':!params.debug}"
          :style="[{ width: pxWidth+'px', height: pxHeight + 'px' }]"></canvas>
...
<script>
   ...
   * @Depend：
   *
   * Created by Hallmader on 2018/9/20.
   */
    ...
    methods: {
      ...
      create() {
        ...
        if (config.backgroundColor) {
          ...
          console.log(this.toPx(config.width), this.toPx(config.height))
          this.ctx.restore();
        }
        ...
        this.ctx.draw(false, () => {
          const width = this.toPx(config.width),
            height = this.toPx(config.height),
            destWidth = this.toPx(config.width)*3,
            destHeight = this.toPx(config.height)*3;
          setTimeout(() => {
            wx.canvasToTempFilePath({
              canvasId: 'myCanvas',
              x: 0,
              y: 0,
              // width: width || this.toPx(750),
              // height: height || this.toPx(1160),
              width:width,
              height:height,
              destWidth: destWidth,
      ...
      initCanvas() {
        this.pxWidth = this.params.width;
        this.pxHeight = this.params.height;
       ...
       */
      drawImage(data) {
        const {imgPath, x, y, w, h, sx, sy, sw, sh, circle = 0, borderRadius = 0, borderWidth = 0, borderColor} = data;
        this.ctx.save();
        console.log(imgPath)
        if (borderRadius > 0) {
          ...
          console.log(circle)
          this.ctx.beginPath();
          this.ctx.arc(this.toPx(x) + this.toPx(w) / 2, this.toPx(y) + this.toPx(w) / 2, this.toPx(w) / 2, 0, 2 * Math.PI);
