...
class WxValidate {
  ...
  formatTplAndAdd(param, rule, value) {
	...
	/**
   * 验证所有字段的规则，返回验证是否通过
   * @param {Object} allValidate 验证的所有数据
   */

	checkAll (allValidate) {
		return new Promise((resolve, reject) => {
			this.__initData();
	
			for (let param in allValidate) {
				this.setView(param)
				this.checkParam(param, this.rules[param], allValidate[param]);
			}
   ...
   /**
   * 验证value的规则，返回验证是否通过
   * @param {String || Number} value 验证数据的值
	 * @param {String} param 验证某个表单
   */
  checkForm(value,param) {
		return new Promise((resolve, reject) => {
			this.__initData();
			this.setView(param)
			this.checkParam(param, this.rules[param], value)
			resolve(this.valid());
		})
  }
  ...
  checkParam(param, rules, value) {
      ...
      for (let method in rules) {
          ...
          if (this.isValidMethod(method)) {
              ...
              // 调用验证方法
              const result = this.methods[method](str, rule.parameters)
              ...
              this.setValue(param, method, result, str)

              // 判断是否通过验证，否则缓存错误信息，跳出循环
              if (!result) {
                  this.formatTplAndAdd(param, rule, str)
                  break
              }
          }
      }
