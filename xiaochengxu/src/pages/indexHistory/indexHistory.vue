<template>
  <section class="main" style="height: 100vh; overflow:hidden;background:#fff;">
    ...
    <section class="header">
      <section class="input-box">
        <div class="sd_searchGroup" data-alcode-mod='759'>
          <img class="search-icon" src="https://m.allinmed.cn/static/image/mp/index/serch_jihuo.png"
               @click.stop="cleanContent">
          <input class="sd_searchInput" data-alcode='e174' :focus="inputFocus" type="text" v-model.trim="searchVal"
                 placeholder="搜索医生、康复日记" @confirm="getSearchResult(1)" @click="gogogo()">
    ...
    <scroll-view v-if="indexShow" class="main-inner1" scroll-y="true"
      ...
      <DoctorItem :organization="doctorList" @FromChild="getErrMsg"></DoctorItem>
      <span class="no-more" v-if="extraShow&&doctorList.length!=0&&toBottom">--到底了--</span>
      <span class="no-more" v-if="extraShow&&doctorList.length!=0&&!toBottom">正在加载...</span>
    </scroll-view>
    <NormalLoading v-if="finish"></NormalLoading>
    <figure class="dr_searchHistory" v-if="!indexShow && historyInfo.length>0">
      <figcaption>历史记录<i class="dr_deleteBtnIcon" @click.stop="confirmShow = true;inputFocus = false"></i></figcaption>
      <ul class="dr_searchList">
        <span v-for="item in historyInfo" class="history-item" @click="getSearchResult(2,item)" :key="index">{{item.keyWord}}</span>
      </ul>
      <transition name="fade">
        <confirm
          :confirmParams="{
      ...
      getHistoryRecord() {
        ...
        getHistoryRecord({
          ...
          if (res && res.responseObject.responseData && res.responseObject.responseData.dataList) {
            this.historyInfo = res.responseObject.responseData.dataList;
          }
        });
      },
      createHistoryRecord() {
