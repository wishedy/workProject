  ...
  <section class="outpatient-order-inner">
    ...
    <section class="outpatient-order-info-content">
      <section class="info-container select-time">
        <ul class="time-list">
          <li class="time-item am" data-alcode='e191' v-if="timeinterval == 'AL' || timeinterval == 'AM'"
              :class="{'selected':timeSeleted==1}" @click="timeSeleted = 1">{{params.month + '-' + params.day}}
            {{params.weekDay}} 上午
          </li>
          <li class="time-item pm" data-alcode='e191' v-if="timeinterval == 'AL' || timeinterval == 'PM'"
              :class="{'selected':timeSeleted==2}" @click="timeSeleted = 2">{{params.month + '-' + params.day}}
            {{params.weekDay}} 下午
          </li>
        </ul>
      </section>
      ...
      <section class="info-container patient-info">
        <ul class="patient-info-list" data-alcode-mod='770'>
          ...
          <li class="patient-info-item" @click="changeNumber()" data-alcode='e189'>
            ...
            <div class="patient-info-item-right" v-if="!changePhone"><span
              class="phone-number">{{telphoneNumber}}</span><span class="change">修改</span>
            </div>
            <div class="patient-info-item-right" v-else>
              <input :focus='changePhone' name="telphoneNumber" v-model="telphoneNumber" class="phone-input"
                     @input="onInput()" @blur="validate()" type="number"
...
<script>
  ...
  const XHRList = {
    updateMobile: api.httpEnv() + "/mcall/customer/patient/baseinfo/v1/updateMobile/", // 更新患者手机号
    createOrder: api.httpEnv() + "/mcall/customer/clinic/v1/create/", // 创建预约单
  };
  ...
  export default {
    ...
    methods: {
      ...
      // 失焦验证
      validate() {
        let isPass = /^1\d{10}$/.test(this.telphoneNumber);
          ...
          beforeSend(config) {
            ...
            if (param.responseObject.responseStatus && param.responseObject) {
              ...
              if (param.responseObject.responsePk == '0') {
                ...
                storage.setItem('sendOrderMsg', JSON.stringify({
      ...
      // 页面初始化
      mountedInit() {
        this.params = JSON.parse(storage.getItem('calenderParams'));
        this.timeSeleted = -1;
    ...
    mounted() {
      ...
      this.targetMsg = JSON.parse(storage.getItem("targetMsg"));
      ...
      &.select-time {
        ...
        .time-list {
          .time-item {
            ...
            &.am {
              &::before {
                background: url("https://m.allinmed.cn/static/image/imScene/am_off.png") no-repeat center;
                background-size: contain;
              }
            }
            &.pm {
              &::before {
                background: url("https://m.allinmed.cn/static/image/imScene/pm_off.png") no-repeat center;
                background-size: contain;
              }
            }
            &::after {
              ...
              position: absolute;
              box-sizing: border-box;
              background: url("https://m.allinmed.cn/static/image/imScene/select_no.png") no-repeat center;
              background-size: contain;
              right: 0;
              top: 50%;
              margin-top: -20px;
            ...
            &.selected {
              ...
              &.am {
                &::before {
                  background: url("https://m.allinmed.cn/static/image/imScene/am_on.png") no-repeat center;
                  background-size: contain;
                }
              }
              &.pm {
                &::before {
                  background: url("https://m.allinmed.cn/static/image/imScene/pm_on.png") no-repeat center;
                  background-size: contain;
                }
              }
              &::after {
                background: url("https://m.allinmed.cn/static/image/imScene/label_succes.png") no-repeat center;
                background-size: contain;
              }
            }
          }
