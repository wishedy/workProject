        ...
        init: function (option) {
            ...
            if (MobileLive.isMobile() == 'isMobile') {
                if ($('#drawPanel').length > 0) {
                    scripts.push("/js/third-party/ccLive/drawingBoard.js");
                }
            } else {
                scripts.push(
            ...
            this.loadScript(scripts, function () {
                ...
                if (MobileLive.isMobile() == 'isMobile' && $.DrawingBoard) {
                    var dp = '<canvas id="drawPanel" width="1200" height="1200" data-has="0"></canvas>'
...
//		直播播放器准备开始播放
    ...
    var MobileLive = {
        ...
        appendVideo: function (s) {
            ...
            var v = '<video  webkit-playsinline playsinline controls autoplay x-webkit-airplay="allow" x5-playsinline width="100%" height="100%" >  '+
                ' <source src="'+s+'?'+timestamp+'" type="video/mp4" />  '+
                ' 你的浏览器不支持H5播放器  '+
                ';/video>  ';
