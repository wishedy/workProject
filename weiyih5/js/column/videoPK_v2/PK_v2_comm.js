...
$(function(){
    //window.onload = Log.createBrowse(52,'视频pk活动2期页面');
    adjustNavWidth();
    //向下滚动nav置顶
    if($(".naver").length>0&&location.href.indexOf('semifinals_result')==-1){
        ...
        $(window).scroll(function(){
            ...
            if(scrollTop+$(window).height()==$(document).height()){
                ...
                for(var i=7;i>=0;i--){
                    if(scrollTop> sTop[i]-$('.resultTag').outerHeight()-50){
    ...
    //获取activityId
    var y = /^[0-9]*$/;
    var activityId =1472178139520;  //线上初赛结果页没有参数，写死
    if(y.test(comm.getpara().activityId)){
        activityId=comm.getpara().activityId;
    }else if(comm.getpara().activityId){
        activityId=comm.getpara().activityId.split('#')[0];
    }
        ...
        $.ajax({
            ...
            success : function(rep){
                if(rep&&rep.responseObject&&rep.responseObject.responseData){
                    $('#activityId').attr('isReviewer',rep.responseObject.responseData.isReviewer);
                    if(rep.responseObject.responseData.isReviewer=="1"){//是专家
                        lia.show();
                        lia.find("a").text("待评作品").attr("href","/pages/column/videoPK_v2/m_assessWorks.html?activityId="+$('#activityId').val());
                    }else{
                        lia.show();
                    }
                }
    ...
    if(location.href.indexOf("primary_result")==-1 && location.href.indexOf("semifinals_result")==-1){    //除初选结果|结果公示页，其他加初选结果
        $(".naver li").eq(0).after('<li><a href="/pages/column/videoPK_v2/primary_result.html?activityId='+$("#activityId").val()+'">初选结果</a></li>');
        adjustNavWidth();
    }
    //活动状态
    $.ajax({
        ...
        success : function(rep){
            if(rep&&rep.responseObject&&rep.responseObject.responseData){
                ...
                if(status==1){//1:结果公示中 -1:过期
                    if(location.href.indexOf("semifinals_result")==-1){//除初选结果|结果公示页，其他加初选结果
                        $(".naver li").eq(0).before('<li><a href="/pages/column/videoPK_v2/semifinals_result.html?activityId='+$("#activityId").val()+'">结果公示</a></li>');
                        adjustNavWidth();
                    }
                }
