...
$(function(){
    //Log.createBrowse(92,'视频选拔-信息完善页');
    if(TempCache.getItem("createdPK")&&location.href.lastIndexOf("m_baoming.html")>-1){
        history.back(-1);
    }
    var data={
    ...
    function doResult(data_list){
       ...
       // var titleShow=userInfo.medicalTitle;
        ...
        if(userName&&titleShow&&company&&email&&mobile){
    ...
    $(".pk_submit").bind("click",function(){
        ...
        if(customerId.length>0&&email.length>0&&mobile.length>0&&titleShow.length>0&&company.length>0&&userName.length>0){
            ...
            if(flage&&flagm&&flagn){
                $.ajax({
                    ...
                    async:true,
                    success:function(data){
                        TempCache.setItem("createdPK",true);
                      g_sps.jump(null,"/html/m/activity/"+$('#activityId').val()+"/activity_index.html?activityId="+$('#activityId').val()+"&access=yes");
                    },
                    error:function(){
                    }
                });
            ...
            user.privExecute({
                ...
                callback: function () {
                    if($(".apply_and_share .applyLeft").text()=="我要报名"){
                        var customerId=localStorage.getItem("userId");
                        if(customerId){
                          g_sps.jump(null,"/pages/column/videoPK_v2/m_baoming.html?activityId="+$('#activityId').val());
                        }
                    }
                }
            });
