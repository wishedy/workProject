    ...
    var PKWork={
        ...
        tagClick:function(){
            ...
            $(".allWorks_select ul li").on("click",function(){
                if(!$(this).hasClass("col_change")){
                    ...
                    if(t.propertyIds){
                        comm.buildAnchor({propertyIds: t.propertyIds,sortIds: t.order});
                    }else{
                        comm.buildAnchor({sortIds: t.order});
                    }
                    t.query();
                    return false;
                }
            ...
            $(".sortList div").on("click", function () {
                ...
                switch(n){
                    case 0:
                        t.order ="publishTime";
                        if(t.propertyIds){
                            comm.buildAnchor({sortIds: t.order,propertyIds: t.propertyIds});
                        }else{
                            comm.buildAnchor({sortIds: t.order});
                        }
                        break;
        ...
        createHtml: function (data) {
            ...
            $(data.rows.items).each(function (i, n) {
               ...
               html += '<section class="run_main_cont_piece" resourceName="'+n.title+'">'+
                    '<section class="run_main_cont_piece_center">'+
                    '<section class="run_main_cont_piece_center_left left">'+
                    '<a href="'+pageUrl+'" style="display:block;"><h3>'+refName+'</h3></a>'+
                    '<ul>'+
                    ...
                    '<a style="display:block;" target="_blank" href="'+pageUrl+'"><img src="'+(imgUrl?imgUrl:'/images/img50/225_150.jpg')+'"></a>'+
                    '<span>'+time+'</span>'+
        ...
        query: function () {
            ...
            data.sessionCustomerId=localStorage.getItem("userId");
            if(t.propertyIds){
                        ...
                        if (data.dataJson.rows.items.length>0) {
                            ...
                            $('.assist').off("click").bind("click",function(){
                                ...
                                if(localStorage.getItem("userId")){
                                    ...
                                    var isPrefer=_that.attr("isPrefer");
                                    if(_that.hasClass("assist_Def")){
                                        t.dianZan(refId,isPrefer);
                                        _that.removeClass("assist_Def").addClass("assist_Win").html("已赞");
                                        var asNum = _that.parents('.run_main_cont_piece_center_left').find('.casePK_support_i');
                                        var asNumA = parseInt(asNum.text())+1;
                                        asNum.text(asNumA);
                                        _that.parents('.run_main_cont_piece_center').find('.zhezhao_dianzanSuc').show().siblings('.zhezhao_dianzanDef').hide();
                                        setTimeout(function(){ _that.parents('.run_main_cont_piece_center').find('.zhezhao_dianzanSuc').hide()},3000);
                                    }else{
                                        if(_that.hasClass("assist_Win")){
                                            t.clearZan(refId,isPrefer);
                                            _that.removeClass("assist_Win").addClass("assist_Def").html("点赞");
                                            var asNum = _that.parents('.run_main_cont_piece_center_left').find('.casePK_support_i');
                                            var asNumC = parseInt(asNum.text())-1;
                                            asNum.text(asNumC);
                                            _that.parents('.run_main_cont_piece_center').find('.zhezhao_dianzanDef').show().siblings('.zhezhao_dianzanSuc').hide();
        ...
        searchVideo:function(){
            ...
            $(".pk_searchBox_right").click(function(){
              g_sps.jump(null,"/html/m/activity/"+$('#activityId').val()+"/personal_works.html?activityId="+$('#activityId').val());
            })
        },
        //搜索:--end
...
$(document).ready(function(){
    $(".search").focus(function () {
      g_sps.jump(null,"/pages/column/videoPK_v2/search_PK_v2.html?activityId="+$('#activityId').val());
    })
})
