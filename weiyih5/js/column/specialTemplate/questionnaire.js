                    ...
                    $('.sp_container [data-id='+Ev.colMenuLayoutModuleId+']').find('.al-questionnaireSelectBtnBox').on("click",function(){
                        ...
                        if(!isFirstClick) {
                            if (_usType == 1 && TempCache.getItem('userId') == null) {//登录权限，未登录  - 登录
                                ...
                                (_usType == 2 && !TempCache.getItem('auth'))||
                    ...
                    $('.sp_container [data-id='+Ev.colMenuLayoutModuleId+']').find('.al-questionnaireSelectBtnBox').on("click",function(){
                        ...
                        if(!isFirstClick){
                            if(_usType==1&&TempCache.getItem('userId')==null){//登录权限，未登录  - 登录
                                ...
                                (_usType == 2 && !TempCache.getItem('auth'))||
                                (_usType==2 && TempCache.getItem('auth') && JSON.parse(TempCache.getItem('auth')).state != 2 && JSON.parse(TempCache.getItem('auth')).state != 7&& JSON.parse(TempCache.getItem('auth')).state != 8&& JSON.parse(TempCache.getItem('auth')).state != 9)
                            ) {//认证权限，去认证
                                user.privExecute({
                                    operateType:'auth',
                                    noNeedBack:true,
            ...
            questionAnswer:function(Rv){  //提交问卷数据
                ...
                $.ajax({
                    ...
                    success : function(data){
                        ...
                        t.doShareHtml(Rv);
                        //本地缓存 已答过的问卷id
                        if(typeof TempCache.getItem('qId') =='string'){ //存在qId
                            TempCache.setItem('qId',TempCache.getItem('qId')+','+Rv.questionnaireId)
                        }else{
                            TempCache.setItem('qId',Rv.questionnaireId)
                        }
                    },
                    error : function(){
                        // comm.LightBox.hideloading();
