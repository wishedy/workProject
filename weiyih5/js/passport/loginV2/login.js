...
$(function () {
    ...
    login={
        ...
        tabCli:function () {//顶部tab切换+一些点击效果
            ...
            $('.registerBtn span').eq(1).on("click",function () {//点击注册按钮
                $('.ev-loginBox').hide();
                $('.ev-registerBox').show();
                $('.ev-loginBox input').val('');
                history.pushState({},"regist","#/regist");
        ...
        loginSubmit:function(data) {
            ...
            if (!result.responseStatus) {         // 登录失败
                ...
                if (comm.isWeiXin()) { //绑定微信操作
                    ...
                    $.ajax({url: wxBindUrl, type: "POST", data: {}}).done(function (rs) {
                        localStorage.removeItem("flagWXRegist");
                    });*/
                } else {
        ...
        registerSubmit:function (account,type,psw,id,validCode){//注册提交
            ...
            var result;
           setTimeout(function () {
               result =CommService.execute("/mcall/web/user/userRegist/?struts.token="+token,param);
