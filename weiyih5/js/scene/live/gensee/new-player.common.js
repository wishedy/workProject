...
function submitQa(){
	...
	if(Util.isNotEmpty(content)){
		...
		$("#qa-text-area").val("");
		loadQaUI({id:"local",question:content,answer:"",submitor:"游客",submitTime:(new Date().getTime()/1000)});
	}
}
...
function loadQaUI(qa){
	...
	var sQaEle = '<li class="qa-'+qa.id+'"><div class="answerMai"><div class="answerRig">';
...
function getQElem(qa){
    ...
    //    <p class="botto">{作者}的问题:{问题?}</p>
    var sQElem ='<p class="botto">游客的问题:'+qa.question+'</p>	';
    return sQElem;
}
//获取答案
function getAElem(qa){
  ...
  //<p class="titl clear"><span>{回答者}</span><span class="bigV"><img src="//m.medplus.net/image/live/bigV.png"alt=""/></span>回答了<i>{时间}</i></p>
    // <p class="middl">{这是答案这是答案}</p>
    var sAElem ='<p class="titl clear"><span>管理员 </span><span class="bigV"><img src="//m.medplus.net/image/live/bigV.png" alt=""/></span>回答了<i>'+Util.formatTime(Number(qa.answerTime)*1000)+'</i></p>' +
        ' <p class="middl">'+qa.answer+'</p>';
