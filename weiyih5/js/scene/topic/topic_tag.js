	...
	topicTag={
		...
		initHandle:function(){
			...
			$("#success").off("vclick").on("vclick",function(){
				t.setLocalStorage();
				//跳转到上传页
        g_sps.jump(null,document.referrer);//pageurl.urlList.toTopicUpload.url;
				return false;
			});
		...
		keyUp:function(){
            ...
            function changeHandler(keyWord){
            	if(keyWord!=""){
					$(".add_tag_li").hide();
					$(".add_tag_li_loading").show();
					var data={userFlag:3,searchParam:$("#seach_tag").val(),pageIndex:1,pageSize:t.pageSize};
					$.ajax({
			...
			function change(ev){
				if($("#seach_tag").val()!=""){
					if(ev.keyCode=="13"){
						t.getTagHtml(0,$("#seach_tag").val());
						$("#seach_tag").val("");
					}
					if(ev.keyCode=="188"&&$("#seach_tag").val().length>1){
						t.getTagHtml(0,$("#seach_tag").val().substring(0,$("#seach_tag").val().length-1));
						$("#seach_tag").val("");
					}
		...
		setLocalStorage:function(){
			...
			if($("#tagList li").length>0){
				...
				localStorage.setItem("tocTagStorage", JSON.stringify(tag));
			}else{
