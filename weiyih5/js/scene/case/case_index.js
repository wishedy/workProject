 ...
 */
$(function(){
    //window.onload = Log.createBrowse(20, "病例列表页面"); //	浏览日志
    comm.footerNav();
    comm.isLiveState();
   var ctrl={
       opt:{
       ...
       path:{
           ...
           getRes:M_CALL+"customer/recommendResourceItem/json_case/",
       ...
       bannerLoad: function() {
           ...
           //comm.loading.show();
           //先加载本地缓存数据，请求结束后替换
           if(TempCache.getItem('caseFirstScreenData')){
               var _tempData = JSON.parse(TempCache.getItem('caseFirstScreenData'));
           ...
           var params = {
               paramJson: $.toJSON({
                   ...
                   channelId: 107, //channelId:''//107-病例 105-视频 106-文库 114-话题  83-首页
       ...
       bannerData:function(data){
           ...
           if (arr.ad_profile_attachment && arr.ad_profile_attachment.length) {
               ...
               //埋点
               $('.Ev-banner a').click(function(){//$('.swiper-slide-active')
           ...
           shareAll({
               ...
               fnMessageSuc:function(){
                   comm.shareLog({
                       ...
                       refId: "",
                       isValid: 1,
                       shareSence:shareSenceConst.case_index,
                       shareChannel:4,
       ...
       dynamicEntry:function(){ //navigationType 导航类型（1-全站导航，2-热门导航，3-发现页按，4-发现页活动专家，5-视频频道页，6-文库频道页，7-话题频道页，8-病例频道页）
           ...
           var params = {paramJson:$.toJSON({navigationType:8,visitSiteId:2,sessionCustomerId:cid,customerId:cid,platformId:TempCache.getItem('department')||1})};
           var callback=function(data){
               comm.loading.hide();
       ...
       topNRec:function(){
           ...
           $.ajax({
               url: t.path.topNList,
               type:"post",
               data:{paramJson: $.toJSON({channelId:7,pageIndex:(t.opt.topNpage+1),pageSize:2,sessionCustomerId:TempCache.getItem('userId')||"",platformId:TempCache.getItem('department')||1 })},
               dataType:'json',
               success:function(d){
       ...
       getRes:function(){
           ...
           var _callback =function(data){
               ...
               if(data&&!$.isEmptyObject(data.responseObject.responseData)&&data.responseObject.responseData.data_list.length){
                   ...
                   TempCache.setItem('caseFirstScreenData',JSON.stringify(t.opt.jsonData));
                   ...
                   t.listDataDispose(item); //成功或者有缓存处理列表数据
                   var itemLength = item.length;
                   if(itemLength<t.params.pageSize){
                       $('.ev_caseBox').attr('scrollPagination','disabled').append(t.noMoreDom());
                   }
                   //t.dynamicEntry();
                   if(itemLength>4){
                       t.topNRec();
