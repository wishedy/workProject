...
//上下文
    var templateDiscuss = {
        reviewList: function (kv, scene) {
            ...
            return '<article class="al-timelineContentItem">' +
                ...
                '<a href="/pages/personal/others_contribution.html?cid=' + kv.customerId + '">' +
                '<img src="' + kv.logoUrl + '" alt="">' +
                '</a>' +
                '</figure>' +
                '<article class="al-timelineContentTextBox">' +
                '<header class="al-timelineContentAuthor">' +
                '<a href="/pages/personal/others_contribution.html?cid=' + kv.customerId + '">' + comm.getStrByteLen(kv.username, 20) + '<i class="al-vipUser"></i><p class="al-timelineTime">' + comm.date.diffDay_one(kv.publishTime,comm.date.local_time()) + '</p></a>' +
                '<span>' + kv.company + '</span>' +

                '</header>' +
                '<article class="al-timelineContentText">' +
                '<a href="javascript:void(0)" style="vertical-align: top;">'+(kv.parentUsername?'回复' + kv.parentUsername + ': ':'')+'</a><span>' + kv.content + '</span>' +
                this.videoH(kv) +
                this.picH(kv) +
                this.quoteH(kv) +
                '</article>' +
        ...
        picH: function (kv) {
            ...
            for (var x = 0, len = imgs.length; x < len; x++) {
                if (len < 7) {
                    imgH += '<figure class="al-timelineImg"><img src="' + imgs[x].attUrl + '" alt=""></figure>';
                }
                if (len > 6 && x < 5) {
                    imgH += '<figure class="al-timelineImg"><img src="' + imgs[x].attUrl + '" alt=""></figure>';
                } else if (len > 6 && x == 5) {
                    imgH += '<figure class="al-timelineImg">' +
                        '<img src="' + imgs[x].attUrl + '" alt="">' +
    ...
    $(function () {
        ...
        //lichunhui update 由于对评论进行回复不能把终端页链接带过来需要请求
        if (refUrl) {
            //更换标题
            ...
            $(".ev-title").html('<a href=' + refUrl + '>《' + (updateTitle != '' ? updateTitle.cutString(24) : _refName.cutString(24)) + '》</a>');
        } else {
            var ajaxUrl = "";
            var typeName = "";
            var paramJson = {};
            switch (refType) {
                case "1"://视频
                    ajaxUrl = M_CALL + 'cms/video/info/';
                    typeName = "cms_video";
                    paramJson = {paramJson: $.toJSON({"videoId": refId})};
                    break;
                case "2"://文库
                    ajaxUrl = M_CALL + 'cms/doc/info/';
                    typeName = "customer_doc";
                    paramJson = {paramJson: $.toJSON({"docId": refId})};
                    break;
                case "4"://话题
                    ajaxUrl = M_CALL + 'cms/topic/base/info/';
                    typeName = "cms_topic";
                    paramJson = {paramJson: $.toJSON({"topicId": refId})};
                    break;
                case "7"://病例
                    ajaxUrl = M_CALL + "case/baseinfo/info/";
                    typeName = "case_baseinfo";
                    paramJson = {paramJson: $.toJSON({"caseId": refId})};
                    break;
                default :
                    break;
            }
            $.ajax({
                ...
                success: function (rep) {
                    if (rep && rep.responseObject.responseData.data_list && rep.responseObject.responseData.data_list.length > 0) {
                        var rows = rep.responseObject.responseData.data_list[0];
                        refUrl = rows[typeName].webStoragePath;
                        //更换标题
                        var _refName = getpara().refName.replace(/@\+@/g, "%");//标题中有% getpara会报错，%用@+@替换下，再换回来
                        var updateTitle = _refName.substring(_refName.indexOf("《") + 1, _refName.lastIndexOf("》"));
                        $(".ev-title").html('<a href=' + refUrl + '>《' + (updateTitle != '' ? updateTitle.cutString(24) : _refName.cutString(24)) + '》</a>');
        ...
        //点击插入视频播放
        $("body").on('click', '.ev-video', function (e) {
            var self = $(this);
            if (self.find('video').length == 0) {
                ...
                //self.append(videoHtml).find('video')[0].play();
        ...
        function upHtmlLoad(html_Arr) {
            ...
            //$('.ev-picList').each(function(ix,elem){
            //    $(elem).find('figure').off('click').on('click', function(){
        ...
        function downHtmlLoad(html) {
            ...
            //$('.ev-picList').each(function(i,ele){
            //    $(ele).find('figure').off('click').on('click', function(){
            //        $('.video-js, video').css('opacity',0);
            //        $(this).picShowComment({
            //            open: $(this).index(),
            //            showVideo:function(){
                ...
                user.privExecute({
                    ...
                    callback: function () {
                        if ($(self).attr('data-collectState') == 0) {
                            ...
                            var num = parseInt($(self).find(".ev-collectNum").text()) - 1;
        ...
        function AjaxAdapter(kv, scene) {
            return {
                ...
                content: kv.customer_review.reviewContent.replace(/&lt;br\/&gt;/g, '<br/>'),
