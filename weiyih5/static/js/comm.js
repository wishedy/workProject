...
const comm = {
    ...
    openBanner:function(config){
      ...
      switch (parseInt(config.resourceType,10)) {
        ...
        case 17://活动
          ...
          break;
        case 19://产品
          _this.openUrl(`https://m.allinmd.cn/dist/medPlus/medPlus.html?#/productDetail?productId=${config.resourceId}`);
          break;
        case 20://品牌
  ...
  loginApp:function(){
    ...
    if(comm.isApp()){
      ...
      console.log('获取到了customerId'+customerId);
      if(customerId){
  ...
  creatEvent: function (opt) {
    ...
    var param = {
      ...
      opt.param? param.param={"bannerUrl":opt.param}:"";//banner的链接（20190329 原本有param字段进行字段调整,数据组许建要求传参）
    if (typeof g_sps != 'undefined') {
      // param = $.extend(param,{
  ...
  getFileSize(target) {
    ...
    // 厂商弹出小黑框提示, title为内容 goback为是否返回首页 vue部分在用这个文件?
    toastFactoryAuth(title, goback){
      if(!title){ title = "该功能仅向医务人员开放"; }

      if($(".ev-toastFactory").length === 0){
        var template = '<section class="ev-toastFactory al-middleTipsBox">'+
            '<section class="al-middleTipsModal ev-commTips show">'+
            '<figure class="al-middleTipsModalText">'+
            '<p class="tipText">'+ title +'</p>'+
            '</figure>'+
            '</section>'+
            '</section>';
        $("body").append(template);
      ...
      function fade(goback){
        $(".ev-toastFactory").fadeIn();
        setTimeout(function(){
          $(".ev-toastFactory").fadeOut();
          if(goback){ window.location.href = '//m.allinmd.cn' }
        },3000);
...
//视频终端页的回复 details_content页需要调这个
