<template>
    <section>
        <div class="row-tow-video-container">
            ...
            <div class="row-tow-video2">
            <div class="video-container">
            <ul>
            <li v-for="(item,index) in RelatedContent"
            :key="item.customerId"
            @click="setPlayerData(item,index)">
            <div class="video2-two-dt"
            :class="active?'active'+index:''">
            <img v-bind:src="item.imageUrls" alt="">
            <span class="originator-box">
            <em class="originator">{{item.videoProtagonist}}</em>
            <em class="originator-remark">{{item.protagonistPosition}}</em>
            </span>
            <i class="play" v-show="playing!==index"></i>
            <i class="pause" v-show="playing===index"></i>
            </div>
            <div class="video2-two-dd">
            <span v-show="playing===index">正在播放：</span><span
            :class="playing!==index?'contentActve':''">{{item.videoTitle}}</span>
            </div>
            </li>
            </ul>
            </div>
        ...
        methods: {
            ...
            getRelatedContent() {
                ...
                axios({
                    ...
                    t.defaultCid = res.data.responseObject.responseData.dataList[0].customerId;
            ...
            showVideo(data, index) {
                ...
                $('.row-two-video1').html('<video preload="auto"  x5-video-player-type="h5-page" id="collegeVideo" playsinline="" x5-playsinline="" webkit-playsinline="" poster="" controls="controls" class="video-js vjs-default-skin vjs-no-flex vjs-big-play-centered"\n' +
