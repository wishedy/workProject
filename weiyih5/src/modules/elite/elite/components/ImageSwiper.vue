    ...
    <section class="al-elite-imageBanner">
        <section class="al-elite-imgContainer swiper-container">
            <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(item,index) in config" :key="index"  :style="{background: 'url('+item.bannerUrl+') no-repeat center center',backgroundSize:'cover'}" @click.stop="openBanner($event)" :slideConfig="JSON.stringify(item)"  @mousedown.stop="trackAction({
            browseName:'广告位',
            actionName:'广告位',
            keyWord:'广告位',
            actionId:'11660',
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            openBanner(e){
                let elementData = JSON.parse($(e.srcElement).attr("slideConfig"));
                EliteSdk.openBanner(elementData);
            },
            trackAction(config){
