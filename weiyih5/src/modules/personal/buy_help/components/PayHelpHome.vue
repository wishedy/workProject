<template>
    <section class="al-payHelp">
        <HeaderBar :header-config="headerConfig" class="al-payHelp-header" v-show="!appPort"></HeaderBar>
        <section class="al-payHelp-container">
            <div>
                <h3 class="pay-questions">
                    <img src="data:image/png;base64," alt="ios支付问题" class="combined-shape">
                    <span class="combined-text">ios支付问题</span>
                </h3>
                <ul>
                    <li v-for="(item,key) in iosList" :key="item.id">
                        <a href="javascript:void (0)" @click="gotoDetail(item.title,item.routeName)">{{item.title}}</a>
                    </li>
                </ul>
            </div>
            <div>
                <h3 class="pay-questions">
                    <img src="data:image/png;base64," alt="支付问题" class="combined-shape">
                    <span class="combined-text">支付问题</span>
                </h3>
                <ul>
                    <li v-for="(item,key) in list" :key="item.id">
                        <a href="javascript:void (0)" @click="gotoDetail(item.title,item.routeName)">{{item.title}}</a>
                    </li>
                </ul>
            </div>
            <div>
                <h3 class="pay-questions">
                    <img src="data:image/png;base64," alt="常见问题"
                         class="combined-shape">
                    <span class="combined-text">常见问题</span>
                </h3>
                <ul>
                    <li v-for="(item,key) in questionList" :key="item.id">
                        <a href="javascript:void (0)" @click="gotoDetail(item.title,item.routeName)">{{item.title}}</a>
                        <span class="arrowUp"></span>
                    </li>
                </ul>
            </div>
            <div class="foot-container" v-show="appPort">
                <span>
                    <!--<img src="../../../../../static/images/payHelp/customs.svg" alt="客服">-->
                </span>
                <a href="javascript:void (0)" @click="concatCustomer()">联系客服</a>
            </div>
        </section>
    </section>
</template>
<script>
    import HeaderBar from '../../../../components/HeaderBar/HeaderBar';

    export default {
        components: {
            HeaderBar
        },
        data() {
            return {
                headerConfig: {
                    title: "购买帮助",  //  *标题项
                    share: {            // ..自定义分享项
                        onOff: false,
                        params: {},
                        authority: 1
                    },
                    feedback: {         //..自定义反馈项
                        onOff: true
                    },
                    backOnOff: true
                },
                iosList: [
                    {id: 1, title: '什么是唯币', routeName: 'AllinmdCurrency'},
                    {id: 2, title: '如何往App内充值', routeName: 'Recharge'},
                    {id: 3, title: '充值的唯币会不会过期', routeName: 'PastDue'},
                    {id: 4, title: '为什么我只能充值指定金额的唯币', routeName: 'OnlyAllinmdCurrency'},
                    {id: 5, title: '余额可以退款或提现吗', routeName: 'RemainingMoney'},
                    {id: 6, title: '为什么更换手机后购买内容还在，而余额不见了', routeName: 'RemainingLose'},
                    {id: 7, title: '通过苹果支付，为什么余额充值后没有立刻到账', routeName: 'RightNow'},
                    {id: 8, title: '为什么提示扣款成功，唯币却没有到账', routeName: 'CurrencyLast'},
                    {id: 9, title: '通过苹果支付，为何实际扣费比充值金额多，是否造成多扣费', routeName: 'Unnecessary'}
                ],
                list: [
                    {id: 1, title: '购买课程如何开发票', routeName: 'payQuestionsDetails'},
                    {id: 2, title: '优惠券使用规则是什么', routeName: 'PayWeChatDetails'},
                    // {id: 3, title: '如何使用支付宝支付', routeName: 'PayZhiFuBaoDetails'}
                ],
                questionList: [
                    {id: 1, title: '支付成功之后无法观看付费课程', routeName: 'AbnormalViewDetails'},
                    {id: 2, title: '关于课程更新', routeName: 'OpenInvoiceDetails'},
                    {id: 3, title: '购买的课程可以下载吗', routeName: 'RefundDetails'},
                    {id: 3, title: '如何查看会员权益', routeName: 'catRight'},
                    {id: 3, title: '会员可以免费听所有付费课程吗', routeName: 'listenBook'},
                    {id: 3, title: '各专业学院课程、手术案例和专栏有什么区别', routeName: 'differentCourse'}
                ],
                appPort: false
            }
        },
        mounted() {
            this.checkApp();
            if (this.appPort) {
                //3.7安卓没写。ios写了，先注释
                appjs && appjs.makeCustomerShareType && appjs.makeCustomerShareType('0');
            }
        },
        methods: {
            gotoDetail(title, routeName) {
                this.$router.push({
                    path: routeName,
                    // query: {title: encodeURIComponent(title)}
                });
            },
            concatCustomer() {
                if (this.appPort) {
                    appjs && appjs.showPhoneCallDialog && appjs.showPhoneCallDialog('0');
                }
            },
            checkApp() {
                let _this = this;
                if (!(typeof appjs == "undefined" || appjs == null || appjs == "")) {
                    return _this.appPort = true;
                } else {
                    return _this.appPort = false;
                }
            }
        }
    }
</script>
