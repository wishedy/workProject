<template>
    <section class="al-payHelp">
        ...
        <section class="al-payDetails-container">
            <h1 class="de-title">充值的唯币会不会过期？</h1>
            <p>在唯医骨科平台的唯币不会过期，会一直存在账户中，您可以随时使用。
            </p>
            <p v-show="appPort">
                仍有问题，
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                headerConfig: {
                    title: "充值的唯币会不会过期",  //  *标题项
                    share: {            // ..自定义分享项
                        onOff: true,
                        params: {
                            sceneType:101,
                            customerId:TempCache.getItem('userId') || '',
                            questionTitle:'充值的唯币会不会过期',
                            questionType:'4'
                        },
                        authority: 1
                    },
        ...
        mounted() {
            ...
            this.headerConfig.title = '充值的唯币会不会过期';
            document.title='充值的唯币会不会过期';
            this.checkApp();
            if (this.appPort) {
                //3.7安卓没写。ios写了，先注释
