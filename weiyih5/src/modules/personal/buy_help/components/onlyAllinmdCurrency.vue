<template>
    <section class="al-payHelp">
        ...
        <section class="al-payDetails-container">
            <h1 class="de-title">为什么我只能充值指定金额的唯币？</h1>
            <p>由于苹果公司规定，苹果设备在充值唯币时，只能选择预设好的充值金额。给您带来的不便敬请谅解。
            </p>
            <p v-show="appPort">
                仍有问题，
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                headerConfig: {
                    title: "为什么我只能充值指定金额的唯币",  //  *标题项
                    share: {            // ..自定义分享项
                        onOff: true,
                        params: {
                            sceneType:101,
                            customerId:TempCache.getItem('userId') || '',
                            questionTitle:'为什么我只能充值指定金额的唯币',
                            questionType:'4'
                        },
                        authority: 1
                    },
        ...
        mounted() {
            ...
            this.headerConfig.title = '为什么我只能充值指定金额的唯币';
            document.title='为什么我只能充值指定金额的唯币';
            this.checkApp();
            if (this.appPort) {
                //3.7安卓没写。ios写了，先注释
