<template>
   <section @click="callApp({text:'更多云云',tplPath:5,type:14})">
       <section class="boneCloud" v-for="(item,index) in boneList" @click="commCreatEvent({id:11264,browType:298,locationId:item.resId})">
           ...
           <section class="contentCon">
               ...
               <section class="oneVideoCon" v-if="item.resAttType==1&&item.resType==22&&item.resAttUrl.length">
                   ...
                   <span v-if="item.resAttUrl[0].picUrl"></span>
               </section>
               <!--笔记-->
       ...
       <button class="moreBoneCloud" v-if="total>3">查看更多</button>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...mapActions(['callApp','commCreatEvent']),
            getByteLen(str){
