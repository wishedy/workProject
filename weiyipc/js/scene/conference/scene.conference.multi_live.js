	...
	$.ajax({
		...
		success : function(data){
			if(data.responseObject.lastName+data.responseObject.firstName){
				User.nickname = data.responseObject.lastName+data.responseObject.firstName+"_"+data.responseObject.customerId+"_1_";
			}else{
				User.nickname = "游客"+"_"+(data.responseObject.customerId?data.responseObject.customerId:0)+"_1_";
			}
...
function changeVideo(obj){
	...
	$("#videoComponent").attr("uname",User.nickname+obj.conSubId);
	$("#videoComponent").attr("ownerid",obj.ownerid);
	
	//video
	$.ajax({     
        url:'//weiyiwang.gensee.com/sdk/site/sdk/video?widgetid=videoComponent&id=videoComponent&site=weiyiwang.gensee.com&ctx=webcast&ownerid='+id+'&uname='+(User.nickname+obj.conSubId)+'&authcode='+playLiveAuthcode,
        dataType:"jsonp",
        jsonp:"jsonpcallback",
	...
	//doc
	$.ajax({
		url:'//weiyiwang.gensee.com/sdk/site/sdk/doc?widgetid=docComponent&id=docComponent&site=weiyiwang.gensee.com&class=gs-sdk-widget&ctx=webcast&ownerid='+id+'&uname='+(User.nickname+obj.conSubId)+'&authcode='+playLiveAuthcode,
        dataType:"jsonp",
        jsonp:"jsonpcallback",
...
function listCtrl(data,src,obj){ //数据控制
	if(src==="con"){ //多会场
		...
		$(".ev-conList").html(html).find("li").on("click",function(){//主导航
			...
			if(!isForeign()) isValidAuth("/html/conference/"+$.getUrlParam("conId")+"/1/meeting_index.html",obj.conSubId);
			//changeVideo(obj);
			//TODO:为了方便记录观看人数点击需要刷新页面
			var href="/pages/channel/conference/multi-conference/live.html?ownerid="+obj.ownerid+"&conId="+$.getUrlParam("conId")+"&conSubId="+obj.conSubId;
