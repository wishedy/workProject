    ...
    $(function () {
        var uploadCase = {
                ...
                $('.ev-submitBtn').off("click").on('click', function () {//按钮点击
                    if (TempCache.getItem("userId")) {
                        if ($('.uploadCont').hasClass('active') && t.config.offClick != false) {//如果按钮是激活状态
                            ...
                            //影像资料验证
                            if (!$(".ev-imageInfo .ev-treatment").find("li").length) {
                                if ($(".ev-caseInfoUp").text() == "收起" && $(".supplement").css("display") != "none") {
                                    t.config.heigh = $(".ev-treatment").parents(".ev-imageInfo").offset().top - 90 - 150;
                                } else {
                                    t.config.heigh = $(".ev-treatment").parents(".ev-imageInfo").offset().top - 90;
                                }
                                $(".ev-imageInfo .title").addClass("error");
                                name = '请上传' + $(".ev-imageInfo .title").text();
                            }
                            //临床转归验证
                            var _clFlag = true;
                            $.each($(".ev-clinical .clinical"), function (i, e) {
                                if ($(e).find("i").hasClass("active")) {
                                    _clFlag = false;
                                    return false;
                                }
                            });
                            if (_clFlag) {
                                if ($(".ev-caseInfoUp").text() == "收起" && $(".supplement").css("display") != "none") {
                                    t.config.heigh = $(".ev-clinical").offset().top - 90 - 150;
                                } else {
                                    t.config.heigh = $(".ev-clinical").offset().top - 90;
                                }
                                $(".ev-clinical").addClass("error");
                                name = '请选择' + $(".ev-clinical .clinicalTitle").text();
                            }
                            //随访记录验证
                            if (!$(".ev-funList").find("li").length) {
                                if ($(".ev-caseInfoUp").text() == "收起" && $(".supplement").css("display") != "none") {
                                    t.config.heigh = $(".ev-addTitleFUN").offset().top - 90 - 150;
                                } else {
                                    t.config.heigh = $(".ev-addTitleFUN").offset().top - 90;
                                }
                                $(".ev-addTitleFUN").addClass("error");
                                name = '请填写' + $(".ev-addTitleFUN .titleText").text();
                            }
                            checkMust($(".ev-health_plan"));//康复计划
                            //手术记录验证
                            if (!$(".ev-funListOp").find("li").length) {
                                if ($(".ev-caseInfoUp").text() == "收起" && $(".supplement").css("display") != "none") {
                                    t.config.heigh = $(".ev-addOpBox").offset().top - 90 - 150;
                                } else {
                                    t.config.heigh = $(".ev-addOpBox").offset().top - 90;
                                }
                                $(".ev-addOpBox").addClass("error");
                                name = '请填写' + $(".ev-addOpBox .titleText").text();
                            }
                            checkMust($(".ev-treatmentRecord"));//治疗过程
                            checkMust($(".diagnosis_identify"));//鉴别诊断
                            checkMust($(".ev-diagnosisInfo"));//诊断依据
                            //诊断检测 验证
                            ...
                            checkMust($(".ev-auxiliary_info"));//辅助检查
                            checkMust($(".ev-caseCheck"));//体格检查
                            checkMust($(".ev-familyHistory"));//家族史
                            checkMust($(".ev-personalHistory"));//个人史
                            checkMust($(".ev-pastHistory"));//既往史
                ...
                comm.ajax.async(t.path.caseUpdate, params, function (rep) {
                    if (rep.responseObject.responseStatus) {//如果上传成功
                        ...
                        if (t.isdraft == 1) {//如果是保存草稿则不跳转
                            ...
                            g_sps.jump(null,"/pages/singlePage/upload-activityCase.html?caseId=" + t.caseId + "&isDraft=1&t=" + $(document).scrollTop());
                        } else {
            ...
            operationName: function () {
                ...
                $(".ev-FUNDetailOp textarea").on("keyup",function () {
                    t.popCheckFunOp();//弹层激活
                });
                //添加手术及记录按钮唤起弹层
                _adBtn.off("click").on("click", function () {
                    if ($(this).attr("data-flag") == 1) {
                        $(".ev-addOpBox").removeClass("error");
            ...
            saveOpFun: function ($this) {
                var t = this,
                    _additionalName = symbolToString($(".ev-opNameText .ev-opName").eq(0).text()),
                    _content = symbolToString($(".ev-opContent textarea").val()),
                ...
                _opNameInput.on("keydown", function (ev) {//8:删除
                    if ($(this).val() == '') {
                        if (ev.keyCode == "8") {
                            ...
                            if ($(".ev-opNameText").find("span").length <= 0) {
                                t.checkAll();
                            }
                        }
                        t.popCheckFunOp();//弹层激活
                        ...
                        comm.ajax.async(t.path.caseOperation, param, function (data) {//数据请求
                            if (data.responseObject.responseStatus) {
                                ...
                                $(".ev-addOpName").off("click").on("click", function () {
                                    var remindHtml = '<span class="atPersonal ev-opName">' + symbolToString(_opNameInput.val()) + '</span>';
                                    if ($(".ev-opName").length) {
                        ...
                        comm.ajax.async(t.path.caseDiagnosis, param, function (data) {//数据请求
                            if (data.responseObject.responseStatus) {
                                ...
                                $(".ev-addDiagnosis").off("click").on("click", function () {
                                    var remindHtml = '<span class="atPersonal ev-diag">' + symbolToString(_diaIn.val()) + '</span>';
                    ...
                    $('.ev-uploadImgCon', $(em)).each(function () {
                        ...
                        _t.uploadImg({
                            ...
                            uploadSuccess: function (data, file) {
                                ...
                                if (!t.uploadImgAdditionanl) {
                                    $(".ev-imageInfo .title").removeClass("error");
                                }
                                var response = data;
            ...
            addAlPopup: function () {
                ...
                $(".ev-addAPBtn").off("click").on("click", function () {
                    if ($(this).attr("data-flag") == 1) {
                        $(".ev-addTitleFUN").removeClass("error");
                        $(this).attr("data-flag", "0");
                        var cBack = function (res) {
                            if (res.responseObject.responseStatus) {
                                $(".ev-alPopup").show();
                        ...
                        comm.ajax.async(t.path.createPop, {paramJson: $.toJSON({'caseId': t.caseId,'additionalType':1})}, cBack);
                    }
                });
                //添加随访记录弹层关闭按钮
                    ...
                    if (res.responseObject.responseStatus) {
                        ...
                        $.each($(".ev-changeBtn"), function (i, e) {
                            if ($this.attr("additionalId") === $(e).attr("additionalId")) {
                                ...
                                $(e).parents("li").find(".detailList").text(_val);
                                _apFlag=false;
                                return false;
                            }
                        });
