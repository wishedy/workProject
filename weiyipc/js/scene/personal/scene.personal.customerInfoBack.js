...
$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(list.authFlag==1){//四证齐全
                content="执业医师审核将在7-15个工作日内完成，请耐心等待";
            }
            //判断头像
    ...
    base = {
        ...
        init: function () {
            ...
            if ( auth.state !== 2) {//未认证
                ...
                /* //职称的选择
                 $("#medical_par").medicalTitleV3({
                 container: $("#med_title"),//存放已选择的职称
                 defaultHide:1
                 });
                 //专业领域
        ...
        deleteSurgery:function(){
                ...
                if(surgeryInput.val().length>0){
                    ...
                    if(surgeryInput.attr("data-editonoff")){
                        ...
                        listObj.each(function(){
                            var dataJson = {
                                operationId: $(this).attr("data-resultsurgeryid"),
                                operationName: $(this).text()
                            };
                            upDateMerge.push(dataJson);
                        });*/
                        if (listObj.length === 0) {
                            surgeryInput.removeAttr("data-editOnOff");
                        }
                        ...
                        /*$.each(surgery.surgeryList,function(i,v){
                            $.each(v, function (ini,inv) {
                                var data = inv.children;
                                for (var dataNumNo = 0; dataNumNo < data.length; dataNumNo++) {
                                    if (deleteId === data[dataNumNo].operationId) {
                                        console.log(i,inv.operationId,data[dataNumNo].operationId)
                                        data[dataNumNo].selectOnOff = false;
                                        break;
                                    }
                                }
                            });
                        ...
                        //console.log(surgery.surgeryList)
            ...
            surgeryInput.off("keyup").on("keyup",function (e) {
                if(e.keyCode===8){
                    editOnOff();
                }
            })
            ...
            inputSurgery.off("keyup").on("keyup", function (e) {
                ...
                if(e.keyCode===8){
                    ...
                    if (len > 0) {
                        ...
                        if((JSON.parse(inputObj.attr("data-continuous")))&&(changeOnOff)){
                            clearTimeout(timer0);
        ...
        bindAuthEdit: function () {
            ...
            $("#base_edit_btn").on("click", function () {
                ...
                console.log(departmentNewResult.val())
                console.log(departmentLastResult.text())
        ...
        saveBaseInfo: function (state) {
            ...
            /*if (departmentStr.length > 0) {
                $(".ev-good-at-department").html(departmentStr);
            }
            if (fieldTypeStr.length > 0) {
                $(".ev-good-at-fieldType").html(fieldTypeStr);
            }*/
            ...
            if (caseOnOff && surgeryOnOff && yearOnOff && mixOnOff) {
                ...
                //console.log(surgery.statisticsSurgery())
                comm.ajax.request(options);
        ...
        fieldTypeOverflow:function () {
            ...
            console.log(fieldTypeContH)
            if (fieldTypeContH > 90){
                fieldTypeMore.show();
        ...
        departmentAutoHeight:function () {
            if (authOnOff){
                InvertedNum($('#departmentInput'),$('.InvertedNum'))
            }else {
                InvertedNum($('#uncertifiedDepartmentInput'),$('.uncertifiedInvertedNum'))
            }
            function InvertedNum(obj,num) {
        ...
        informationNone:function () {
            ...
            textNull(departmentLastResult)//科室
            textNull(fieldTypeLastResult)//擅长领域
            textNull(yesteryearNumLastResult)//去年的手术数为空
            textNull(precedingNumLastResult)//去年的手术数为空
        },
        ...
        bindAuthSave: function (auth) {
            // 改版前代码
            var t = this;
            /*$("#base_authSave").on("click",function(){
             var data={};
             data.resumeType = languageFlag;
             data.company=auth.company;
             data.medicalTitle=auth.medicalTitle;
             data.areasExpertise=auth.areasExpertise;
             $(".ev-authsexId i").each(function (i, em) {
             if ($(em).hasClass("al-tableRadioOn")) {
             data.sexId = $(em).parent().attr("sexId");
             }
             });
             if ($("#authYear option:selected").val() != "0" && $("#authMonth option:selected").val() != "0" && $("#authDay option:selected").val() != "0") {
             data.birthday = $("#authYear option:selected").val() + '-' + $("#authMonth option:selected").val() + '-' + $("#authDay option:selected").val();
             } else {
             data.birthday = "";
             }
             $("#base_authEdit").mask("");
    ...
    doctorWrote = {
        init: function () {
            ...
            $("#doctorWrote_hid").val(baseInfo.sendWord);
        ...
        save: function () {
            function restoreDom() {
                ...
                $('#doctorWrote_count').text($('#doctorWrote').val());
