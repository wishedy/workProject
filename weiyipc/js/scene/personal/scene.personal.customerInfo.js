...
$(function () {
    ...
    var path = {
        ...
        getDataRoleConfigs: PC_CALL + "comm/data/roleconfig/getSortList/",
        getAuthAttachments: PC_CALL + "customer/auth_attachment/getAuthAttachmentsV2/",
        // getAuthAttachments: PC_CALL + "customer/auth_attachment/getAuthAttachments/",
        // getAuthAttachments: PC_CALL + "customer/auth_attachment/getMapList2/",
...
//                    t.submitConfig();
    ...
    var professional = {
        init: function (type) {
            ...
            var professionalList = $(".al-personal-center-professional .al-tableItemProfessional");
            professionalList.off("click").on("click", function () {
                var t = $(this);
                if (t.index() != $(".al-personal-center-professional .al-tableItemProfessional.active").index()) {
                    var areaData = '';
                    $("#ev-areaSed .tagName").each(function () {
                        var _isThis = $(this);
                        var strName = _isThis.attr('tagid') + "_" + _isThis.find("span").text();
                    ...
                    if (t.index() === 0) {
                        selectSepcial.areasTempList['' + 1] = areaData;
                        if (selectSepcial.areasTempList['' + 2].length) {
                            selectSepcial.areasInit(selectSepcial.areasTempList['' + 2]);
                        } else {
                            $("#ev-areaSed").addClass('al-selectDefault').html('请选择专科');
                        }
                    } else {
                        selectSepcial.areasTempList['' + 2] = areaData;
                        if (selectSepcial.areasTempList['' + 1].length) {
                            selectSepcial.areasInit(selectSepcial.areasTempList['' + 1]);
                        } else {
                            $("#ev-areaSed").addClass('al-selectDefault').html('请选择专科');
                        }
                    }
                    ...
                    professional.platformId = (t.index() + 1);
                    selectSepcial.areasDataList = [];
                    //$("#ev-areaSed").html('');
                    selectSepcial.areasList(professional.platformId);
                }
            }).each(function (i) {
                if (i + 1 == type) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            //需要更新缓存中的customerRole
            if (unit != null && unit != undefined) {
                TempCache.setItem("customerRole", unit.customerRole);
            }
            ...
            professional.init(auth.platformId);
            selectSepcial.areasInit(auth.areasExpertise);
            selectSepcial.areasList(auth.platformId);
            $(".ev-professionalShow").html(auth.areasExpertiseShow);
            $(".ev-specialtyShow").html(auth.platformId == 1 ? "骨科" : '手外科');
            ...
            if (auth.state !== 2 && auth.state !== 7 && auth.state !== 8 && auth.state !== 9) {//未认证
                ...
                }//如果角色是医助或厂商，不可以申请变更，隐藏申请变更按钮
                else if(unit && (unit.customerRole==="3" || unit.customerRole==="13")){
                    $(".ev-applyAuth").hide();
                }
                else {
            ...
            $(".ev-birthdayShow").html(birthYear);//基本资料出生能日期
            $(".ev-birth").text(birthYear ? (birthYear.substring(0, 4) + '年' + birthYear.substring(5, 7) + '月' + birthYear.substring(8, 10) + "日") : '');
            if (birthYear) {
                $("#birthdayYear").find("option[value='" + birthYear.substring(0, 4) + "']").attr("selected", true);
                $("#birthdayMonth").find("option[value='" + birthYear.substring(5, 7) + "']").attr("selected", true);
                $("#birthdayDay").find("option[value='" + birthYear.substring(8, 10) + "']").attr("selected", true);
            } else {
                gettime($("#birthdayYear"), '1980');//年
                gettime($("#birthdayMonth"), '01');//月
                gettime($("#birthdayDay"), '01');//日
            }
    ...
    applyAuth = {
        init: function () {
            ...
            //3.5迭代添加的逻辑，如果是厂商/医助角色，则不显示“申请”变更的按钮
            ...
            $(".ev-applyAuth").on("click", function () {
                ...
                if (auth.state === 8) {
                    ...
                    comm.surePop({
                        icon: 0,
                        title: '<img src="/images/img10/authentication/guid_v.png">',
                        content: '认证审核需要1-3个工作日，在此期间您无法在线执业\n' +
                        '是否继续变更？',
                        contentCss: contentCss,
                        passCss: 'margin-top:0',
                        sureClass: 'sureClass',
                        btnText: '立即变更',
                        success: function () {
                            edit();
    ...
    base = {
        ...
        comparisonArr: function (oriArr, lastArr) {
            var same = false;
            if (oriArr.length != lastArr.length) {
                same = true;
            } else {
                for (var i = 0; i < oriArr.length; i++) {
                    var hasOnOff = false;
                    for (var j = 0; j < lastArr.length; j++) {
                        if (oriArr[i] == lastArr[j]) {
                            hasOnOff = true;
                        }
                    }
                    if (!hasOnOff) {
        ...
        saveNewAdditionBaseInfo: function () {
            ...
            var sameProfessional = true;
            var sameSpecialty = true;
            var resultBirthday = $("#birthdayYear option:selected").text() + '-' + $("#birthdayMonth option:selected").text() + '-' + $("#birthdayDay option:selected").text();
            var resultProfessional = $(".al-tableItemProfessional.active").index() + 1;
            var resultSpecialty = [];
            var resultSpecialtyStr = '';
            var resultSpecialtyNameStr = '';
            $(".al-tableTagConfigItem.tagName").each(function () {
                var _t = $(this);
                resultSpecialty.push(_t.attr('tagid') + '_' + _t.find('span').text());
                if (resultSpecialtyStr.length == 0) {
                    resultSpecialtyNameStr += _t.find('span').text();
                    resultSpecialtyStr += _t.attr('tagid') + '_' + _t.find('span').text();
                } else {
                    resultSpecialtyNameStr += "," + _t.find('span').text();
                    resultSpecialtyStr += "," + _t.attr('tagid') + '_' + _t.find('span').text();
                }
            });
            sameSpecialty = t.comparisonArr(selectSepcial.areasDataList, resultSpecialty);
            sameBirthday = !(resultBirthday == ($.trim(baseInfo.birthday) ? baseInfo.birthday : auth.birthYear));
            sameProfessional = !(resultProfessional == professional.platformId);
            var param = {
                "customerId": obj.cid,
                "birthYear": resultBirthday,
                "platformId": resultProfessional,
                "areasExpertise": resultSpecialtyStr,
                visitSiteId: 1
            };
            // debugger;
            if ((sameBirthday || sameProfessional || sameSpecialty) && (resultSpecialtyStr.length != 0)) {
                professional.platformId = (resultProfessional);
                selectSepcial.areasDataList = resultSpecialty;
                $(".ev-birthdayShow").html(resultBirthday);
                $(".ev-professionalShow").html(resultSpecialtyNameStr);
                $(".ev-specialtyShow").html(resultProfessional == 1 ? "骨科" : '手外科');
                //comm.ajax.async("/call/call/customer/auth/updateBaseinfo/",{"paramJson": $.toJSON(param)},callBack);
            }
            return param;
        },
        saveBaseInfo: function (state) {
            ...
            function callBack() {
                var birthDay = jsonAdd['birthYear'].split('-');
                selectSepcial.areasTempList = {1: '', 2: ''};
                $(".ev-hasAuth .al-tableItem .ev-birth").html(birthDay[0] + '年' + birthDay[1] + '月' + birthDay[2] + '日');
                comm.ajax.request({
                    "port": "/call/customer/auth/getCustomerAuth/",
                    postData: {
                        customerId: obj.cid
            ...
            function initArea(data) {
                var htmlStr = '';
                var listArea = data.split(',');
                $.each(listArea, function (index, value) {
                    var tagData = value.split('_');
                    var tagId = tagData[0];
                    var tagName = tagData[1];
                    htmlStr += '<figure class="al-tableItemTagItem" tagid="' + tagId + '">' + tagName + '</figure>';
                });
                return htmlStr;
            }
            ...
            var jsonAdd = t.saveNewAdditionBaseInfo();
            if (caseOnOff && surgeryOnOff && yearOnOff && mixOnOff) {
                $('#base_authEdit').hide();
                $('#base_show').show();
                $('#base_edit_btn').show();
                if (jsonAdd) {
                    jsonAdd['visitSiteId'] = 1;
                    jsonAdd['customerId'] = obj.cid;
                    comm.ajax.async("/call/customer/auth/updateBaseinfo/", {"paramJson": $.toJSON(jsonAdd)}, function () {
                        $('.ev-hasAuth .al-tableItem .ev-areas').html(initArea(jsonAdd['areasExpertise']));
                        var birthDay = jsonAdd['birthYear'].split('-');
                        selectSepcial.areasTempList = {1: '', 2: ''};
                        $(".ev-hasAuth .al-tableItem .ev-birth").html(birthDay[0] + '年' + birthDay[1] + '月' + birthDay[2] + '日');
                    });
                ...
                if (jsonAdd) {
                    $('.ev-hasAuth .al-tableItem .ev-areas').html(initArea(jsonAdd['areasExpertise']));
                    param['birthYear'] = jsonAdd['birthYear'];
                    param['platformId'] = jsonAdd['platformId'];
                    param['areasExpertise'] = jsonAdd['areasExpertise'];
                }
        ...
        /*手术病例数*/
        //3.5迭代要求隐藏，目前仅做了前端css的display:none操作
        ...
        bindAuthSave: function (auth) {
            ...
            $("#base_authSave").off("click").on("click", function () {
                ...
                // if (departmentNewResult.val() == ''
                //     || fieldTypeNewResult.val() == ''
                //     ||  $('.al-adept-case-name').length <= 0
                //     || $('.al-adept-surgery-name').length <= 0) {
                //     return false
                // }
    ...
    //通讯地址
    //3.5迭代需求，为了和APP端统一，隐藏通讯地址,通过css设置display:none 实现
    var contactAddress = {};
    contactAddress = {
        init: function () {
            //既然不显示，就不进行后续操作了，省点性能，后期如果需要展示，再打开即可
    ...
    authAttachment = {
        ...
        bindAuthAttachment: function () {
            ...
            var param = {
                "isValid": 2,
                "visitSiteId": 1,
                medicalTitleId: auth.authMedicalId
            };
            $.ajax({
                type: 'POST',
                    ...
                    if (rep && rep.responseObject && rep.responseObject.responseStatus) {
                        ...
                        var imgDemoList = [
                            ...
                            },
                            {
                                ...
                                refValue: "医师职称证书",
                                "refId": "13",
                                "ImgList": ["//img10.allinmd.cn/authentication/certificate/pic_zhicheng.png"],
                                startOnOff: true
                            },
                            {
                                refValue: "工作证(学生证)",
                                "refId": "10",
                                "ImgList": ["/images/img10/authentication/certificate/pic_work.png"],
                                startOnOff: false
                            },
                            {
                                refValue: "工作证",
                                "refId": "11",
                                "ImgList": unit && unit.customerRole ==="12"?["/images/img10/authentication/certificate/pic_work_nurse.png"]:["/images/img10/authentication/certificate/pic_work.png"], //护士的工作证示例图片和医生的不一样
                                startOnOff: false
                            },
                            {
                                refValue: "学生证",
                                "refId": "12",
                                "ImgList": ["/images/img10/authentication/certificate/pic_student.png"],
                                startOnOff: false
                            },
                            {
                                "refId": "7",
                            ...
                            },
                            {
                                ...
                                refValue:"护士执业证",
                                "refId":"15",
                                "ImgList":["/images/img10/authentication/certificate/pic_nurse_zhiye1.png","/images/img10/authentication/certificate/pic_nurse_zhiye2.png"],
                                startOnOff: true
                            },
                            {
                                refValue: "医学学历证",
                                "refId": "16",
                                "ImgList": ["/images/img10/authentication/certificate/pic_nurse_zige.png"],
                                startOnOff: false
                            }
                        ];
                        ...
                        if (data && data.length > 1) {
                            $.each(data, function (i, val) {
                                ...
                                html += '<li class="al-certificate-item" data-auth-atttype="' + val.refId + '" data-auth-demo="' + imgDemoStr + '">' +
                                    ...
                                    //     //8执业；6资格
                                    //     if(val.refId==8){
                                    //         str='<span class="onlyTipRed">需包含必要信息页：基本信息页和变更信息页</span>';
                                    //     }
                                    //     return str;
                                    // }())+
                                    '	</div>' +
        ...
        exhibitionCertificate: function (data) {
            ...
            var templateImg = function (imgData) {
                ...
                $.each(imgData, function (i, v) {
                    if (v.attPath && (v.attPath.indexOf('jpg') != -1 || v.attPath.indexOf('jpeg') != -1 || v.attPath.indexOf('png') != -1 || v.attPath.indexOf('PNG') != -1 || v.attPath.indexOf('JPG') != -1 || v.attPath.indexOf('JPEG') != -1)) {
            ...
            for (var dataNum = 0; dataNum < data.length; dataNum++) {
                ...
                if (imgArr["" + v.attType]) {
                    ...
                    imgArr["" + v.attType] = [];
                    var attPath = v.attPath ? v.attPath : imgObj.find(".al-certificate-result").eq(0).find('img').attr("src");
                    var firstJson = {
                        "attPath": v.attPath ? (v.attPath.lastIndexOf("allinmd.cn") > -1 ? v.attPath : "//img05.allinmd.cn/" + v.attPath) : attPath,
                        "attPositionType": v.attPositionType,
                        "id": (v.id) ? v.id : "",
                        attCode: v.attCode,
                        createTime: timestamp
            ...
            $.each(uniqueImgArr, function (i, v) {
                ...
                if (imgListData) {
                    if (imgObj.find(".al-certificate-img").length === 0) {
                        ...
                        imgObj.find(".al-certificate-img").remove();
                        imgObj.find(".al-certificate-header-bar").after(templateImg(uniqueImg(imgListData)));
                    }
            ...
            if (imgCerNum) {
               //如果是护士角色，则不显示补全四证的提示
                if(unit.customerRole === "12"){
                    $('.al-auth-certificate-reminder').hide();
                }
                else{
            ...
            postObj.find(".al-certificate-edit-ensure").off("click").on("click", function () {
                ...
                if ($(this).hasClass("active")) {
                    ...
                    imgContainer.find(".al-certificate-post-item").each(function () {
                        if ((!$(this).hasClass("al-certificate-post-example")) && ($(this).find(".al-certificate-post").attr("pic"))) {
                            var jsonImageData = {
                            ...
                            imgListData.push(jsonImageData);
                            postImageList.push(jsonImageData);
                        } else if ((!$(this).hasClass("al-certificate-post-example")) && ($(this).find(".al-certificate-result img").attr("src"))) {
                            var jsonImageData = {
                                "attCode": postObj.find(".al-certificate-number-input").val(),
                                "attPath": $(this).find(".al-certificate-result img").attr("src"),
                                "attPositionType": 1,
                                "attType": nowAttrType,
                                "id": "_"
                            };
                    ...
                    imgContainer.find(".al-certificate-post").each(function (i) {
                        ...
                        param = {
                            "customerId": obj.cid,
                            authAttList: JSON.stringify(postImageList),
                            isCompleted: 1,
                            opflag: 2
                        };
        ...
        bindEdit: function () {
            ...
            $(".al-certificate-upload").off("click").on("click", function () {
                ...
                var callBack = function () {
                    var options = {
                        ...
                        failed: function () {
                            ...
                            //2019-03-18 3.5迭代中产品要求把所有填写编码的地方都去掉
                            var numBerOnOff = false;
                            ...
                            for (var imgNum = 0; imgNum < len; imgNum++) {
                                ...
                                if (desNum === 0) {
                                    if (len > 3) {
                                        var picTitle;
                                        // if(parseInt(imgObj.attr("data-auth-attType"),10)===8){
                                        //     if(oddNumber==0){
                                        //         picTitle='基础页+照片页示例';
                                        //     }else if(oddNumber==1){
                                        //         picTitle='信息页+变更页示例';
                ...
                comm.ieAlert("", '继续看看在说', callBack, false);
            ...
            obj.uploadPic.uploadImg({
                ...
                uploadSuccess: function (serverJson, file) {
                    ...
                    if (serverJson.responseObject.responseStatus) {
                        ...
                        $.extend(obj, {picpath: serverJson.responseObject.responseMessage.url});
                        t.bindUpload(obj);
                        obj.uploadPic.parent('.al-certificate-post-item').css({
                            'background': 'url(' + obj.picpath + ') no-repeat 0 0',
                            'backgroundSize': '100% 100%'
                        });
                        obj.uploadPic.find("input[type='file']").remove();
    ...
    /*医生寄语开始*/
    //3.5迭代需求，为了和APP端统一，隐藏医生寄语,通过css设置display:none 实现
    var doctorWrote = {};
    doctorWrote = {
        init: function () {
            //既然不显示，就不进行后续操作了，省点性能，后期如果需要展示，再打开即可
