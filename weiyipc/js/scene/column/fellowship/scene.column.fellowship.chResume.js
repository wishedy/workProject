...
var chResume=function(obj){
    ...
    var path={
        ...
        controller= {
            ...
            init:function(obj){
                ...
                $("#ch_resume").show().attr("data-fId", t.config.fellowshipId);
                $(".Ev-ChResumePreview").attr("href","/pages/column/fellowship/fellowship_nowResumePreview.html?cId="+$('#sesCustomerId').val()+"&fId="+t.config.fellowshipId+"&lanFlag="+languageFlag);
                this.par=this.options.type=="ch"?"#ch_resume ":"#en_resume ";
              /*  if(!$("#ch_resume").attr("hasClick")) {
                    $("#ch_resume").attr("hasClick",1);*/
                    $.ajax({
                        type: "post",
                        url: path.unitInfo,
                ...
                //}
                //展开和收起按钮
                $(t.par+".per_bj_floor").each(function(i,em){//展开和收起
                ...
                $(t.par+".resume_close").off("click").on("click",function(){//关闭弹层
                    ...
                    if(!$(this).attr("data-flag")==1){
                        $.ajax({
                            ...
                            data: {
                                id:closeBaseInfo.id,
                                resumeType:0,
                                fellowshipId:$(this).parents("#ch_resume").attr("data-fId"),
                                sexId:closeBaseInfo.sexId,
                                birthday:closeBaseInfo.birthday,
                                medicalTitle:closeBaseInfo.medicalTitle,
                                company:closeBaseInfo.company,
                                nativePlace:closeBaseInfo.nativePlace,
                                nativePlaceId:closeBaseInfo.nativePlaceId,
                                nationality:closeBaseInfo.nationality,
                                nationalityId:closeBaseInfo.nationalityId,
                                politicsStatus:closeBaseInfo.politicsStatus,
                                politicsStatusId:closeBaseInfo.politicsStatusId,
                                firstName:closeBaseInfo.firstName,
                                lastName:closeBaseInfo.lastName
                            },
                            dataType: "json",
                            success: function (rep) {
                    ...
                    var _fId=$(this).parents("#ch_resume").attr("data-fId");
                    base.clearData();
                    ...
                    var _editBtn=$($(".Ev-ApplyInfoShowOrHide[data-fId="+_fId+"]")).find(".chResume_btn");
             ...
             /*       if($.trim($(t.par+ ".name").text()).length>0&&$(t.par+ ".sex").text().length>0&&$(t.par+ ".birth").text().length>0
                        &&$.trim($(t.par+ ".native_place").text()).length>0&&$.trim($(t.par+ ".nationality").text()).length>0
                        &&$.trim($(t.par+ ".politics_status").text()).length>0&&$(t.par+ ".medical").text().length>0
                        &&$(t.par+ ".company_show").text().length>0&&$(t.par+ ".summary_count").text().length>0
                        &&$(t.par+ ".occ_list").length>0&&$(t.par+ ".edu_list").length>0){
            ...
            //鼠标悬浮显示选择照片函数
            addMask: function () {
                $("#imgfile_ch").parent().append('<div class="layerChoose imgMask" style="display: none">'+
                    '<div class="chooseImgT"><img src="//img00.allinmd.cn/column/fellowship/sc_img_tishi.png"></div>'+
                    '<div class="chooseImgC font_yahei">选择照片</div>'+
                    '<div class="chooseImgB">只支持JPG '+'\u002f'+' PNG，大小不超过5M</div>'+
                    '</div>');
                $("#imgfile_ch").parent().bind("mouseover", function () {
            ...
            uploadPic:function(picpath){
                ...
                $("#ch_logo").html('<input type="file" id="imgfile_ch" name="file" />');

                //}
                var paramJson = $.toJSON({imageType:1,logoType:11,domain:location.hostname});
                czyx.uploadReplace('#imgfile_ch', {
                    ...
                    uploadBefore: function () {
                        ...
                        var fileSize = comm.file.getFileSize(document.getElementById("imgfile_ch"));
                        if (fileSize > 5242880) {
                    ...
                    uploadEnd: function (serverJson) {//上传完毕后调用
                        try {
                            ...
                            if (serverJson.responseObject.responseStatus) {
                                ...
                                t.addMask();
                                $("#en_logo div").eq(0).css({"background-image":"url("+serverJson.responseObject.responseMessage.url+")"});
                            } else {
    ...
    base = {
        ...
        getList: function () {
            ...
            var path = {};
            path={
                getNativePlace:PC_CALL+"commdata/getNativePlaceList/",//籍贯
                getNationality:PC_CALL+"commdata/getNationalityList/",//民族
                getPoliticsStatus:PC_CALL+"commdata/getPoliticsStatusList/"//政治面貌
            };
            //把籍贯数据放到籍贯容器的函数
            var getNative = function (data) {
                var html="";
                if(data){
                    $.each(data,function(i,val){
                        html+="<option value='"+val.regionName+"' data-id='"+val.id+"'>"+val.regionName+"</option>"
                    })
                    $(t.par+".layerTran .native").append(html);
                };
            };
            //把民族数据放到籍贯容器的函数
            var getNation = function (data) {
                var html="";
                if(data){
                    $.each(data,function(i,val){
                        html+="<option value='"+val.nationality+"' data-id='"+val.id+"'>"+val.nationality+"</option>"
                    })
                    $(t.par+".layerTran .nation").append(html);
                };
            };
            //把政治面貌数据放到籍贯容器的函数
            var getPolitics = function (data) {
                var html="";
                if(data){
                    $.each(data,function(i,val){
                        html+="<option value='"+val.politicsStatus+"' data-id='"+val.id+"'>"+val.politicsStatus+"</option>"
                    })
                    $(t.par+".layerTran .politics").append(html);
                };
            }
            t.getListFun(path.getNativePlace,getNative);
            t.getListFun(path.getNationality,getNation);
            t.getListFun(path.getPoliticsStatus,getPolitics);
        },
        //获取籍贯，民族，政治面貌的列表封装的函数
        getListFun: function (pathUrl,fun) {
            var t = this;
            $.ajax({
                type: "POST",
                url:  pathUrl,
        ...
        getInfo: function () {
            ...
            lastName= ($.trim(fellowship.lastName)&&!$.isEmptyObject(fellowship.lastName))?fellowship.lastName:auth.lastName;
            firstName= ($.trim(fellowship.firstName)&&!$.isEmptyObject(fellowship.firstName))?fellowship.firstName:auth.firstName;
            medicalTitle= ($.trim(fellowship.medicalTitle)&&!$.isEmptyObject(fellowship.medicalTitle))?fellowship.medicalTitle:auth.medicalTitle;
            company= ($.trim(fellowship.company)&&!$.isEmptyObject(fellowship.company))?fellowship.company:auth.company;
            closeBaseInfo.id=baseInfo.id;
            closeBaseInfo.lastName=lastName;
            closeBaseInfo.firstName=firstName;
            closeBaseInfo.sexId=baseInfo.sexId;
            closeBaseInfo.birthday=baseInfo.birthday;
            closeBaseInfo.medicalTitle=medicalTitle;
            closeBaseInfo.company=company;
            closeBaseInfo.nativePlace=baseInfo.nativePlace;
            closeBaseInfo.nativePlaceId=baseInfo.nativePlaceId;
            closeBaseInfo.nationality=baseInfo.nationality;
            closeBaseInfo.nationalityId=baseInfo.nationalityId;
            closeBaseInfo.politicsStatus=baseInfo.politicsStatus;
            closeBaseInfo.politicsStatusId=baseInfo.politicsStatusId;

            if (lastName || firstName) {
                $(t.par+ ".name").text(lastName + firstName);
            }
            //$(t.par+".region").text(baseInfo.city=="县"?baseInfo.province:baseInfo.city);
            if (baseInfo.sexId == "1") {
                $(t.par+ ".sex").text("男");
            }
            if (baseInfo.sexId == "2") {
            ...
            $(t.par+ ".birth").text(baseInfo.birthday);

            $(t.par+ ".medical").text(comm.cutLine(medicalTitle, "_", "，"));
            $(t.par+ ".company_show").text(company);
            $(t.par+ ".register_time").text(baseInfo.registTime.substring(0, 10));
			$(t.par+ ".native_place").text(baseInfo.nativePlace);
			$(t.par+ ".nationality").text(baseInfo.nationality);
			$(t.par+ ".politics_status").text(baseInfo.politicsStatus);
            if (auth.state >= 0) {
                    ...
                    success: function (rep) {
                        if (rep && rep.responseObject.responseData) {
                            ...
                            if (auth.state >= 0) {
                                ...
                                $(t.par+ ".firstName").val(firstName);
                                $(t.par+ ".lastName").val(lastName);
                            } else {
                            ...
                            //自动选中籍贯的选项
                            $.each($(t.par+".layerTran .native option"),function(i,em){
                                if($(em).val()==baseInfo.nativePlace){
                                    $(em).attr("selected",true);
                                }
                            });
                            
                            //自动选中民族的选项
                            $.each($(t.par+".layerTran .nation option"),function(i,em){
                                if($(em).val()==baseInfo.nationality){
                                    $(em).attr("selected",true);
                                }
                            });
                            
                            //自动选中政治面貌的选项
                            $.each($(t.par+".layerTran .politics option"),function(i,em){
                                if($(em).val()==baseInfo.politicsStatus){
                                    $(em).attr("selected",true);
                                }
                            });
                            ...
                            $(t.par+ ".med_title").text(_medicalTitle.substring(0, _medicalTitle.length - 1));
                            //清空职称的选中状态
                            ...
                            $(t.par+ ".company_in").val(company);

                            //医院的搜索下面是jQuery联想医院的插件
                            $(t.par+ ".company_in").lenovo({
                                url: path.hospital,
        ...
        bindSave: function (state) {
            var t=this;
            var has=0;
            $(t.par+ ".base_save").off("click").on("click", function () {
                if(has){
                    return;
                }
                var data = {};
                ...
                data.fellowshipId=$(this).parents("#ch_resume").attr("data-fId");
                //data.nickname=$(t.par+".nickName").val();
                //if (!state) {
                    data.firstName = $(t.par+ ".firstName").val();
                    data.lastName = $(t.par+ ".lastName").val();
                //}
                //城市函数备用
                ...
                data.medicalTitle = $(t.par+ ".med_title").attr("medicaltitle");
                data.company = $(t.par+ ".company_in").val();
				
				//获取籍贯的信息
				data.nativePlace = $(t.par+".layerTran .native option:selected").val();
				data.nativePlaceId = $(t.par+".layerTran .native option:selected").data("id");
				//获取民族的信息
				data.nationality = $(t.par+".layerTran .nation option:selected").val();
				data.nationalityId = $(t.par+".layerTran .nation option:selected").data("id");
				//获取政治面貌的信息
				data.politicsStatus = $(t.par+".layerTran .politics option:selected").val();
				data.politicsStatusId = $(t.par+".layerTran .politics option:selected").data("id");
				
                var isSuccess = 1;
                /*if(!$.trim(data.nickname)){
                 $(t.par+ ".nickName").parent().addClass("baseinfo_error_red");
                 $(t.par+ ".nick_error").show();
                 isSuccess = 0;
                 }else{
                 $(t.par+ ".nickName").parent().removeClass("baseinfo_error_red");
                 $(t.par+ ".nick_error").hide();
                 }*/
                
                //验证籍贯
                if(!data.nativePlaceId){
                    $(t.par+ ".native_error").show();
                    isSuccess = 0;
                }else{
                    $(t.par+ ".native_error").hide();
                }
                //验证民族
                if(!data.nationalityId){
                    $(t.par+ ".nation_error").show();
                    isSuccess = 0;
                }else{
                    $(t.par+ ".nation_error").hide();
                }
                //验证政治面貌
                if(!data.politicsStatusId){
                    $(t.par+ ".politics_error").show();
                    isSuccess = 0;
                }else{
                    $(t.par+ ".politics_error").hide();
                }
                ...
                $.ajax({
                    ...
                    success: function (rep) {
                        ...
                        if (rep.rows.responseStatus) {
                            ...
                            $(t.par+ ".base_edit").hide();
                            $(t.par+".up_down").eq(0).attr("edit",0);
                            //$(t.par+ ".nick").text(data.nickname);
                            //$(t.par+ ".region").text(data.city=="县"?data.province:data.city);
                            if (data.sexId == "1") {
                                $(t.par+ ".sex").text("男");
                                $("#en_resume .sex").text("Male");
                            }
                            if (data.sexId == "2") {
                                $(t.par+ ".sex").text("女");
                                $("#en_resume .sex").text("Female");
                            }
                            $(t.par+ ".birth").text(data.birthday ? data.birthday.substring(0,7) : '');
                            var birth=getEnTime({'date':data.birthday});
                            $("#en_resume .birth").text(birth.year?(birth.year+'.'+birth.month):'');
                            $(t.par+ ".medical").text($(t.par+ ".med_title").text());
                            $(t.par+ ".company_show").text(data.company);
							$(t.par+ ".native_place").text(data.nativePlace);
							$(t.par+ ".nationality").text(data.nationality);
							$(t.par+ ".politics_status").text(data.politicsStatus);
                            if (state >= 0) {
                                if (!data.birthday) {
                                    $(t.par+ ".birth").parents(".p_base_info_jbxx01").hide();
                                } else {
    ...
    summary = {
        init: function (obj) {
            ...
            $(t.par+".summary_hid").val(baseInfo.summary);
            $(t.par+".summary_count").text($(t.par+".summary_hid").val());
            if($(t.par+".summary_count").text()) {
                $(t.par + ".summary_count").expandShrink({len: 728});
            }
    ...
    occ={
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="occ_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="occ_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_bao_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    '<div class="p_baseinfo_hospital unit_show">'+options.unit+'</div>'+
                    '<div class="p_baseinfo_hospital_zl"><span class="department_show">'+options.department+'</span><span>｜</span><span class="medicalTitle_show">'+options.medicalTitle+'</span><span>｜</span><span class="occupation_show"'+(!options.occupation?"style='display:none'":'')+'>'+options.occupation+'</span><span '+(!options.occupation?"style='display:none'":'')+'>｜</span><span class="time00">'+options.time+'</span></div>'+
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="occ_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="occ_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
                ...
                $.ajax({
                    ...
                    success : function(rep){
                        if(rep&&rep.responseObject.responseData.bo_data){
                            ...
                            $(t.par+".department").val(data.department);
        ...
        save:function(){
            ...
            $(t.par+".occ_save").die("click").live("click",function(){
                ...
                if(!id){//添加
                    data.customerId=t.customerId;//客户ID
                }else{//修改
                    data.id=id;
                }
                data.unit=$(t.par+".unit").val();
                data.department=$(t.par+".department").val();
                ...
                var sTime=$(t.par+".occ_start_year option:selected").val()+'.'+$(t.par+".occ_start_month option:selected").val();
                data.startTime=$(t.par+".occ_start_year option:selected").val()+'-'+$(t.par+".occ_start_month option:selected").val();
                var eTime=$(t.par+".occ_end_year option:selected").val()+'.'+$(t.par+".occ_end_month option:selected").val();
                ...
                var isSuccess=1;
                //验证    $.trim()是jQuery提供的函数，用于去掉字符串首尾的空白字符
                if(!$.trim(data.unit)){
                ...
                if(!$.trim(data.department)){
                    $(t.par+".department").parent().addClass("baseinfo_error_red");
                    $(t.par+".department").parent().next().show().text("请选择科室或部门!");
                    isSuccess=0;
                }else if(comm.getByteLen(data.department)>50){
                    $(t.par+".department").parent().addClass("baseinfo_error_red");
                    $(t.par+".department").parent().next().show().text("最长25个中文或50个英文!");
                    isSuccess=0;
                }else{
                    $(t.par+".department").parent().removeClass("baseinfo_error_red");
                    $(t.par+".department").parent().next().hide();
                }
                if(!data.medicalTitle){
                    ...
                    success : function(rep){
                        ...
                        if(rep.responseObject.responseStatus){
                            /*if(id){
                                ...
                                $obj.find(".department_show").text(data.department);
    ...
    edu={
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="edu_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="edu_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_mao_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    ...
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="edu_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="edu_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
                    ...
                    if(data.responseObject.responseData.data_list&&data.responseObject.responseData.data_list.length>0) {
                        ...
                        $.each(list,function(i,val){
                            ...
                            html+= t.templete.returnList({
                                ...
                                education:htmlToString(val.education),
        ...
        save:function(){
            ...
            $(t.par+".edu_save").die("click").live("click",function(){
                ...
                var sTime=$(t.par+".edu_start_year option:selected").val()+'.'+$(t.par+".edu_start_month option:selected").val();
                data.startTime=$(t.par+".edu_start_year option:selected").val()+'-'+$(t.par+".edu_start_month option:selected").val();
                var eTime=$(t.par+".edu_end_year option:selected").val()+'.'+$(t.par+".edu_end_month option:selected").val();
    ...
    cEdu={
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="cEdu_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="cEdu_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_book_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    ...
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="cEdu_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="cEdu_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
                    ...
                    if(data.responseObject.responseData.data_list&&data.responseObject.responseData.data_list.length>0) {
                        ...
                        $.each(list,function(i,val){
                            ...
                            html+= t.templete.returnList({
                                ...
                                time:time,
                                cmeDesc:htmlToString(val.cmeDesc),
                                id:val.id
                            });
                        })
                    };
        ...
        clear:function(){
            ...
            /*$(".cEdu_time").show();
             $("#cEdu_upNow").attr("value",0);
             $("#cEdu_upNow").removeClass("p_c_baseinfo_check_hover");*/
        },
        //编辑
        bindEdit:function(){
            var t=this;
        ...
        save:function(){
            ...
            $(t.par+".cEdu_save").die("click").live("click",function(){
                ...
                var sTime=$(t.par+".cEdu_start_year option:selected").val()+'.'+$(t.par+".cEdu_start_month option:selected").val();
                data.startTime=$(t.par+".cEdu_start_year option:selected").val()+'-'+$(t.par+".cEdu_start_month option:selected").val();
                var eTime=$(t.par+".cEdu_end_year option:selected").val()+'.'+$(t.par+".cEdu_end_month option:selected").val();
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="honor_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="honor_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_jb_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    ...
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="honor_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="honor_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="fund_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="fund_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_s_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    ...
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="fund_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="fund_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
    ...
    social={
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="social_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="social_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_jt_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    '<div class="p_baseinfo_hospital organization_show">'+options.organization+'</div>'+
                    ...
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="social_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="social_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
        ...
        save:function(){
            ...
            $(t.par+".social_save").die("click").live("click",function(){
                ...
                var sTime=$(t.par+".social_start_year option:selected").val()+'.'+$(t.par+".social_start_month option:selected").val();
                data.startTime=$(t.par+".social_start_year option:selected").val()+'-'+$(t.par+".social_start_month option:selected").val();
                var eTime=$(t.par+".social_end_year option:selected").val()+'.'+$(t.par+".social_end_month option:selected").val();
    ...
    opus={
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="opus_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="opus_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_gz_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    ...
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="opus_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="opus_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
        ...
        save:function(){
            ...
            $(t.par+".opus_save").die("click").live("click",function(){
                ...
                var time=$(t.par+".pubYear option:selected").val()+'.'+$(t.par+".pubMonth option:selected").val();
    ...
    patent={
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="patent_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="patent_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_d_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
                    ...
                    '</div>'+
                    '<div class="baseinfo_del"><a href="javascript:;" class="patent_delBtn" val="'+options.id+'">删除</a></div>'+
                    '<div class="baseinfo_bj"><a href="javascript:;" class="patent_editBtn" val="'+options.id+'">编辑</a></div>'+
                    '<div class="clear"></div>'+
                    '</div></div>';
                return html;
            }
        ...
        save:function(){
            ...
            $(t.par+".patent_save").die("click").live("click",function(){
                ...
                var time=$(t.par+".patYear option:selected").val()+'.'+$(t.par+".patMonth option:selected").val();
        ...
        templete:{
            returnList : function(options){
                ...
                html+='<div class="language_list p_c_title_bj_zl baseinfo_after_mr">'+
                    '<div class="language_show"><div class="p_c_title_bj_zl_l font_yahei layerNoMargin">'+
                    '<div class="bj_d_bg"></div>'+
                    '</div>'+
                    '<div class="p_c_title_bj_zl_r font_yahei">'+
...
//          });
        ...
        getLanguage:function(){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(data){
                        $.each(data,function(i,val){
                            html+="<option value='"+val.languageName+"' data-id='"+val.id+"'>"+val.languageName+"</option>"
                        })
        ...
        save:function(){
            ...
            $(t.par+".language_save").die("click").live("click",function(){
                ...
                var isSuccess=1;
				//验证
			   	if(!$.trim(languageName) && "其他" == language){
