    ...
    questionLoad:function(Qv){
        var questionnaire={
            ...
            questionDataLoad:function(Kv){
                var t=this;
                if(Kv.resTemplate.bo.responseData&&Kv.resTemplate.bo.responseData.data_list&&Kv.resTemplate.bo.responseData.data_list.length>0){
                    var qItem=Kv.resTemplate.bo.responseData.data_list;
            ...
            doQueResult:function(Lma) {
                ...
                $($('.sp_container').find('div[data-id=' + Lma.colMenuLayoutModuleId + ']')).find('.resource_list').html(_totalHtml).attr({userType:Lma.questionData.userType,customerStatusQuestionnaire:Lma.questionData.customerStatusQuestionnaire,qId:Lma.questionData.questionnaireId});
                $($('.sp_container ').find('div[data-id=' + Lma.colMenuLayoutModuleId + ']')).find('.al-questionBox').append(quesBtn);
            ...
            questionAnswer:function(Rv){  //提交问卷数据
                ...
                $.ajax({
                    url : PC_CALL+"activity/question/answer/create/",
                    type : "POST",
                    data :  params,
                    async : false,
                    time : 5000,
            ...
            questionResult:function(Lrv){  //查看投票数据
                ...
                var params={paramJson: $.toJSON(reData)};
                $.ajax({
                    url : PC_CALL+"activity/question/answer/getCQASpecialCount/",
                    type : "POST",
                    data :  params,
                    async : false,
                    time : 5000,
            ...
            questionnaireShare: function(qnSv) {
                var themeId = $('#activityId').val(),// "1480647316221";
