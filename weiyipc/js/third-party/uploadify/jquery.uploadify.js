...
(function($) {
			...
			return this.each(function() {
				...
				if (flashInstalled) {
					...
					swfuploadify.queueData = {
						...
						errorMsg           : '只允许上传.jpg .jpeg .png类型的图片文件'
					};
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '只允许上传.jpg .jpeg .png类型的图片文件';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件名为 "' + file.name + '" 已经在上传队列,要重新上传吗?');
					if (!replaceQueueItem) {
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '\n选定的文件数量超过限制数量 (' + settings.queueSizeLimit + ').';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n选定的文件 "' + file.name + '" 超过限制大小 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n选定的文件 "' + file.name + '" 为空.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n选定的文件 "' + file.name + '" 不是限制的类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('文件名为 "' + file.name + '" 已经在上传队列,要重新上传吗?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
