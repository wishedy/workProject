					...
					for(var i=0,l=dataItem.length;i<l;i++){
						var item = dataItem[i],
							...
							shareBtn_suc = "<li class='gr_pointer'></li>"+
								...
								"<div class='fenxiang'>"+
								"<span>分享至：</span>"+
								"<a href='javascript:;' class='shareToWeibo'><img src='//www.allinmd.cn/image/img00/column/videoPK/weibo.png' alt='' /></a>"+
								"<a href='javascript:;' class='shareWx'><div class='Pk_ewm_cont'><div class='pk_ewm_text'>使用微信扫描二维码</div></div><img src='//www.allinmd.cn/image/img00/column/videoPK/wechat.png' alt='' /></a>"+
								"<a href='javascript:;' class='shareQQ'><img src='//www.allinmd.cn/image/img00/column/videoPK/gr_qq.png' alt='' /></a>"+
								"<a href='javascript:;' class='shareQzone'><img src='//www.allinmd.cn/image/img00/column/videoPK/kongjian.png' alt='' /></a>"+
								"</div>"+
								"</li>",
...
//						3:如果状态为有效则有链接，有分享、判断审核状态
							...
							if(item.customer_trends.isValid===1){										//视频有效
								deadLink="//www.allinmd.cn"+item.customer_trends.headerUrl;
...
//						2：病例有效则有链接与分享
					...
					if($('#activityId').attr('isResultShowBegin')){//如果结果公示开始//0:未知错误 1:结果公示中 -1:过期
						...
						$('#gr_upload_now')
							...
							.off('click')
							.attr('href',"//www.allinmd.cn/pages/column/videoPK/pk4_results.html");
					}else{
						...
						if($('#activityId').attr('isReviewBegin')==1||$('#activityId').attr('isSignUpOver')==-1){
							...
							$('#gr_upload_now')
								...
								.off('click')
								.attr('href',"//www.allinmd.cn/pages/column/videoPK/personal_works.html");
						}else{
							module.videoCasePk({
	...
	if($("#sesCustomerId").val()){//已登录
		...
		if($('#activityId').attr('isResultShowBegin')){//如果结果公示开始//0:未知错误 1:结果公示中 -1:过期
			...
			$('#login_now').css({'background':'url("//img00.allinmd.cn/column/videoPK/gr_upl04.png") no-repeat'}).off('click').attr('href',"//www.allinmd.cn/pages/column/videoPK/pk4_results.html");
		}else{
			if($('#activityId').attr('isReviewBegin')==1){//如果（评分开始），点击查看参赛作品// 1:开始 -1:未开始
				...
				$('#login_now').css({'background':'url("//img00.allinmd.cn/column/videoPK/gr_upl03.png") no-repeat'}).off('click').attr('href',"//www.allinmd.cn/pages/column/videoPK/personal_works.html");
			}else{//在上传未截止之前，点击登录
