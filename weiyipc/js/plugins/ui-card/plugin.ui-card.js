    ...
    var controller = {
        ...
        showCard: function () {
            ...
            timeout = setTimeout(function () {
                ...
                domEl = (typeof options.content === "function")? options.content():options.content;   // 获取内容
        ...
        calculPos: function () {
            ...
            var leftArrowPos = pos.left + elementWidth / 2 - 35 + options.offsetLeft ;   // 箭头居左，卡片主要居右时
            var rightArrowPos = pos.left + elementWidth / 2 + 35 -options.width + options.offsetLeft; //箭头居右，卡片主要居左时
            var bottom = pos.top + 6 + elementHeight + options.offsetTop;       // 卡片位于下部时
            var top  = pos.top - 6 - options.height + options.offsetTop;      // 卡片位于上部时
            switch(className){
                case "top-left":
                    t.popCard.css({
                       top:top,
