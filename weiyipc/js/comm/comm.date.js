 ...
 * 使用方法: a 发布时间 b 当前时间
				 ...
				 dataJoint(startTime,endTime,sign,middleJoint) 活动持续时间 sign默认为点，可用‘/’如2016.10.1-10.3或2016/10/1-10/3
 																middleJoint默认“-”【中间连接】 如2016-01-02/2016-02-03
...
comm.date = {
		...
		thirdRule:function(dateStrBefore, dateStrAfter){
		//第三种展示规则
            ...
            if(d<1){//秒分小时
                ...
                if(d<8){
                    ...
                    var year = comm.date.local_time().split("-")[0];
		...
		vsLocalTime:function(time){//日期格式 2016-10-18 23:20
			...
			var tSu = (now - thatTime )/1000;//秒
			if(tSu>0){
				var _d = tSu/(60*60*24);//天
				var _a = tSu/(60*60);	//小时
				var _m = tSu/60;		//分钟
				if(_d>1){
					return parseInt(_d)+"天"+parseInt(_a%24)+'小时'+parseInt(_m%24)+'分钟';
				}else if(_a>1){
					return parseInt(_a)+'小时'+parseInt(_m%24)+'分钟';
				}else if(_m>1){
					return parseInt(_m)+'分钟'
				}else{
					return parseInt(tSu)+'秒';
				}
			}/*else{
				var _d = tSu/(60*60*24);//天
				var _a = tSu/(60*60);	//小时
		...
		dateLocalJoint: function (date1, date2, sign, middleJoint) {//date1开始时间，date2结束时间,sign要替换时间的符号，middleJoint两个时间中间的连接符号
			var result;
			var signal = (sign != undefined && sign != "") ? sign : '.';               //
			var mJoint = (middleJoint != undefined && middleJoint != "") ? middleJoint : "-";
			var d1 = date1.substring(0, 10).replace(/-/g, signal);
