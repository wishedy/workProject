...
gulp.task("sass-pc", function() {
    ...
    pipe(gulp.dest("./css/v3/"));
});

//Ψҽh5
gulp.task("watch-h5", function() {
    gulp.watch("../workspaces/allin_h5V2_static/scss/**/*.scss", ['sass-h5']);
});

gulp.task("sass-h5", function() {
    return gulp.src("../workspaces/allin_h5V2_static/scss/**/*.scss").
    pipe(sass({ outputStyle: "expanded" ,indentWidth:4,indentType:'space'})).
    // pipe(cleancss({
        // advanced: false,
        // keepBreaks: false,
    ...
    pipe(gulp.dest("../workspaces/allin_h5V2_static/css"));
});
