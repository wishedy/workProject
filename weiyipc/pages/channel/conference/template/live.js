...
$(function(){
	...
	var dialog=function(opt){
		...
		html+='<div id="remind_shade" style="position:absolute; left:0; top:0; background:#ffffff; opacity:0.6; filter:alpha(opacity=60); -moz-opacity: 0.6;"></div>'+
	    	'<div class="message" style="background:#000; position:absolute; opacity:0.6; filter:alpha(opacity=60); -moz-opacity: 0.6; padding:8px 58px 8px 58px;">'+
	    		'<div class="ico" style="float:left;">'+ (o.pic?'<img src="//img.caos-china.org/zhmm/g_success.png" style="vertical-align:middle;">':'') +'</div>'+
	...
	Common.showloading = function(){
	    this.loading = (this.loading || $('<div style="position:absolute; top:45%; left:50%;z-index:18; display:none" id="ctrollerLoading"><img src="//img.caos-china.org/shade/xubox_loading0.gif" alt="loading" /></div>').appendTo(document.body)).show();
...
//-----------------------------------------------------------------------------------------	
	...
	//playType 播放类型 live为0直播 playback为回放
	...
	if(playType.length == 0) playType=0;
	...
	var backURL = "";
	if(playType == 0){
		backURL = "/html/conference/"+conId+"/1/meeting_index.html";
	}else{
		backURL = "/html/conference/"+conId+"/meeting_index.html";
	}
	
	//判信息是否完整
	backPrevURL = backURL;
	var infoUrl = PC_CALL+"customer/auth/getAttendVideoStatus/";
	$.ajax({
		url:infoUrl,
		data:{},
		type:"post",
		success : function(data){
			if(!data.responseObject.responseStatus) location.href=backPrevURL;
		}
	})
	...
	function isStartLive(){
		var conferenceTime = "2015/01/12 08:00:00";
	...
	function tear(c,t){
		...
		if(days>0){
			days = "<span style=\"color:#3a8dcb;font-size:25px;\">"+days+"</span>天"
		}else{
			days = "";
		}
		if(hours>0){
			...
			$.ajax({
				...
				success : function(data){
					if(data != null && data.responseObject != undefined){
						user.nickname = data.responseObject.lastName+data.responseObject.firstName;
